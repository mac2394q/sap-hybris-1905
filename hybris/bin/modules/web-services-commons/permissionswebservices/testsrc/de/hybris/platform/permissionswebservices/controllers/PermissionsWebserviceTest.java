...
public class PermissionsWebserviceTest extends AbstractPermissionsWebServicesTest
{
	...
	private static final String BASE_PATH ="permissions/principals";

	static
	{
		ALL_FALSE_PERMISSIONS = new HashMap<>();
		...
		ALL_TRUE_PERMISSIONS = new HashMap<>();
		...
		READ_TRUE_PERMISSIONS = new HashMap<>();
	...
	@Test
	public void shouldReturnAllTypesAccessesForAdmin() throws JAXBException
	{
	...
	@Test
	public void shouldReturnsOtherUserPermissiondForAdmin() throws JAXBException
	{
		//when retrieving type permissions
	...
	{
		//given
		WsRequestBuilder requestBuilder = new WsRequestBuilder()//
				.extensionName(PermissionswebservicesConstants.EXTENSIONNAME)//
				.path(VERSION);
		//when
			...
			final CatalogPermissionsListWsDTO tested)
	{
		...
		expected.getPermissionsList().forEach(p -> Assert.assertTrue(tested.getPermissionsList().stream()//
				.anyMatch(t -> areCatalogPermissionsWsDTOEquals(p, t))));
	}
