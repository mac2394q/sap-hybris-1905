...
public class CacheControlHandlerInterceptorTest
{
	...
	private final CacheControlController cacheController = new CacheControlController();
	private final PrivateCacheController privateCacheController = new PrivateCacheController();
	...
	public void testPrivateOnlyUsingClassAnnotation() throws Exception
	{
		...
		Assert.assertEquals("private", response.getHeader(HEADER_CACHE_CONTROL));
	}
	...
	public void testPublicOnlyWhenMethodAnnotationOverridesClassAnnotation() throws Exception
	{
		...
		// then
		Assert.assertNotNull(response.getHeader(HEADER_CACHE_CONTROL));
		Assert.assertEquals("public", response.getHeader(HEADER_CACHE_CONTROL));
	}
