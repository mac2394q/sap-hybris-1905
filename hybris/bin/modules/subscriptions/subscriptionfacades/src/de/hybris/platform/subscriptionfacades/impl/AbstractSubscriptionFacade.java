...
public abstract class AbstractSubscriptionFacade implements SubscriptionFacade
{
	...
	public Date getSubscriptionEndDate(@Nonnull final ProductData subscriptionProductData, @Nullable final Date startDate)
	{
		...
		else
		{
			...
			frequencyCode = termOfServiceFrequency == null ? "null termOfServiceFrequency" : termOfServiceFrequency.getCode();
		}
	...
	public AbstractOrderEntryModel getOrderEntryForOrderCodeAndEntryNumber(@Nonnull final String orderCode, final int entryNumber)
	{
		...
		{
			LOG.debug("Order or entry not found, returning null", e);
			return null;
		}
	}
	...
	@Override
	@Nonnull
	public List<SubscriptionData> getSubscriptionsForPaymentMethod(@Nullable final String paymentMethodId) throws SubscriptionFacadeException
	{
		...
		paymentSubscriptions.addAll(allSubscriptions.stream()
				.filter(subscriptionData -> StringUtils.equals(subscriptionData.getPaymentMethodId(), paymentMethodId))
				.collect(Collectors.toList()));
	...
	@Override
	@Nonnull
	public CCPaymentInfoData updateCreatedPaymentMethod(@Nonnull final CCPaymentInfoData paymentInfo, @Nonnull final Map<String, String> paramMap) 
			throws SubscriptionFacadeException
	{
		...
		try
		{
			...
			if (merchantPaymentMethodId != null)
			{
				...
				{
					LOG.warn(String.format("Overwriting subscriptionId: %s --> %s", model.getSubscriptionId(), merchantPaymentMethodId));
				}
