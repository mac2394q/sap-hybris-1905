...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask(taskName, function() {
        ...
        const webpackConfig = serialize(config.data, {
            ...
            .replace(/%commonsManifestFile%/, config.commonsManifestFile ? config.commonsManifestFile : null);

        grunt.log.warn("Deprecated since 1905. Please use 'smartedit-build/builders' instead. For more information on upgrading " +
    ...
    function validateConfig(webpackConfig) {
        ...
        if (webpackConfig.tsConfigFile && !webpackConfig.tsConfigShared && fs.existsSync(webpackConfig.tsConfigShared)) {
            ...
            const {
                config,
                error
            } = ts.readConfigFile(configFilePath, path => fs.readFileSync(path, 'utf-8'));
            if (error) {
