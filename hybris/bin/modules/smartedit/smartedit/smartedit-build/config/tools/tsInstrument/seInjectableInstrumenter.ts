...
module.exports = function(fileNames: string[], injectableDecorators: string[]) {
	...
	const DECLARATIONS_REGEXP = /declarations[\s]*:[\s]*\[([^\[\]]+)\]/;
	const ENTRY_COMPONENTS_REGEXP = /entryComponents[\s]*:[\s]*\[([^\[^\]]+)\]/;
	...
	function walkNode(node: ts.Node) {
		...
		if (node.kind === ts.SyntaxKind.ClassDeclaration && ts.isClassDeclaration(node)) {
			...
			if (classDeclaration.decorators && classDeclaration.decorators.length) {
				...
				if (lodash.intersection(classDecorators.map((d) => d.name), injectableDecorators).length) {
					const fileName: string = node.getSourceFile().fileName;
					const className: string = node.name.getFullText().trim();
					const intersectDecorators = classDecorators.filter((c) => injectableDecorators.indexOf(c.name) > -1);
	...
	function getTemplaterUrlRegexp() {
		// return /templateUrl\s*:(\s*['"`](.*?)['"`]\s*([,}]))/gm;
		return /templateUrl:[\s\S]*(['"`]{1}[\.\/a-zA-Z0-9]+['"`]{1})/;
	}
	...
	function serializeDecorator(decorator: ts.Decorator): DecoratorData {
		...
		const originalArguments = decorator.expression.getFullText().replace(/^[a-zA-Z]+\(/, "").replace(/\)$/, "");
		...
		return {
			name,
			originalArguments,
			arguments: shouldReplaceTemplateUrl ? originalArguments.replace(getTemplaterUrlRegexp(), "template: require($1),") : originalArguments,
			shouldReplaceTemplateUrl
		};
	}

	function getTransformedSource(node: ts.Node, fileName: string, className: string, classDecorators: DecoratorData[]): string {
		...
		classDecorators.forEach((d) => {
			if (d.arguments && !isDecoratorIgnored(d.name)) {
				...
				fullText = fullText.substr(0, index) + `\r(window as any).__smartedit__.addDecoratorPayload('${d.name}', '${className}', ${d.arguments});` + fullText.substr(index);
			}
		...
		if (classDecorators.find((d) => d.name.startsWith("Se") || d.name === 'Injectable')) {
			return fullText.replace(new RegExp("(export class " + className + ")", 'g'), `\rexport class ${DELEGATING_HINT} ${className}`);
		} else {
			return fullText;
		}
	}
