...
exports.config = {
    ...
    onPrepare: function() {
        ...
        browser.dumpLogs = function() {
            return browser.manage().logs().get('browser').then(function(browserLogs) {
                browserLogs
                    ...
                    .filter(function(log) {
                        return !isStaticResourceFailure(log);
                    })
        ...
        function isStaticResourceFailure(log) {
            return log.message.includes("Failed to load resource");
        }
        ...
        browser.waitForContainerToBeReady = function() {
            ...
            return browser.wait(protractor.ExpectedConditions.elementToBeClickable(element(by.css('[data-item-key="headerToolbar.userAccountTemplate"] button'))), EXPLICIT_WAIT, "could not find user account toolbar menu when first loading app").then(function() {
        ...
        browser.click = function(source, errorMessage) {
            ...
            return browser.waitUntil(protractor.ExpectedConditions.elementToBeClickable(this._getElementFromSource(source)), message).then(function() {
        ...
        browser.waitForSelectorToContainValue = function(selector, text) {
            return browser.waitUntil(EC.textToBePresentInElementValue(browser._getElementFromSource(selector), text), "selector " + selector + " does not contain expected text " + text);
        };
        ...
        browser.clearLocalStorage = function() {
        ...
        browser.testThatOverflowingContentIsHidden = function(source) {
            ...
            return element.getCssValue("height").then(function(height) {
                var expectedHeight = Number(height.replace("px", ""));
                expect(element.getAttribute("scrollHeight")).toBeGreaterThan(expectedHeight);
            });
        };
        ...
        browser.bringElementIntoView = function(source) {
            return browser.executeScript('arguments[0].scrollIntoView(false)', browser._getElementFromSource(source).getWebElement());
        };
