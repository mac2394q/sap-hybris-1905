...
class JQueryHelper {
	...
	wrap(name: string, ...elementsArray: ElementForJQuery[]): jasmine.SpyObj<JQuery> {
		...
		const mockedMethodsOfJQueryWrapperNames = !!elementWithMocks ? lo.uniq(lo.cloneDeep(this.DEFAULT_MOCKED_JQUERY_FUNCTIONS).concat(Object.keys(elementWithMocks.mockedMethodsOfJQueryWrapper))) : this.DEFAULT_MOCKED_JQUERY_FUNCTIONS;

		const elementsWrapper = jasmine.createSpyObj<any>(name, this.IDEMPOTENT_METHODS.concat(mockedMethodsOfJQueryWrapperNames));
		...
		if (elementWithMocks) {
			lo.forEach(elementWithMocks.mockedMethodsOfJQueryWrapper, (value: Primitive, methodName: Extract<keyof JQuery, string>) => {
