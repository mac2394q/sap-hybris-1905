 ...
 */
module.exports = (tsconfig, transpileOnly = true, cache = false, enableHappyPack = true) => group(
    rule({
        test: /\.ts$/,
        sideEffects: true,
        use: [{
    ...
    enableHappyPack && happyPack('ts', 2, [...(cache ? [cacheLoader] : []), {
        path: 'ts-loader',
        query: {
            happyPackMode: true,
            configFile: tsconfig,
    ...
    transpileOnly && plugin(new ForkTsCheckerWebpackPlugin({
