...
import {MUTATION_TYPES} from 'smarteditcommons/utils';
...
export class InViewElementObserver {
	...
	addSelector(selector: string, callback?: () => void): () => void {
		if (!this.isBlank(selector) && this.selectors.map((el) => el.selector).indexOf(selector) === -1) {
			if (/\.[\w]+/.test(selector)) {
		...
		if (!this.mutationObserver) {
			...
			if (!this.intersectionObserver) {
				...
				this.intersectionObserver = this._newIntersectionObserver((entries) => {
					 ...
					 * 
					 */
					this.selectors.filter((element) => !!element.callback).forEach((element) => {
	...
	private _aggregateAddedOrRemovedNodes(mutations: MutationRecord[], addedOnes: boolean) {
		const entries = lodash.flatten(mutations.filter((mutation) => {
			...
			const children = lodash.flatten(Array.prototype.slice.call(addedOnes ? mutation.addedNodes : mutation.removedNodes)
				.filter((node: HTMLElement) => {
					return node.nodeType === Node.ELEMENT_NODE;
				})
				.map((child: HTMLElement) => {
					const eligibleChildren = this._getAllEligibleChildren(child);
					return this._isEligibleComponent(child) ? [child].concat(eligibleChildren) : eligibleChildren;
				}))
	...
	private _aggregateMutationsOnClass(mutations: MutationRecord[], addedOnes: boolean) {
		return lodash.compact(mutations.filter((mutation: MutationRecord) => {
			...
			if (addedOnes && this._isEligibleComponent(mutation.target as Element)) {
				return mutation.target;
			} else if (!addedOnes && this._isEligibleComponent(mutation.target as Element)) {
				return mutation.target;
			}
			return null;
		}));
	...
	private _newMutationObserver(callback: MutationCallback) {
		...
		mutationObserver.observe(document.body, lodash.merge(lodash.cloneDeep(IN_VIEW_ELEMENTS_MUTATION_OBSERVER_OPTIONS), {attributes: this.hasClassBasedSelectors}));
