...
export class CacheEngine {
	...
	public addItem(item: ICacheItem<any>, cacheTiming: ICacheTiming, refresh: <T>() => Promise<T>): void {
	...
	protected refreshCache<T>(obj: ICacheItemRegistry): Promise<T | void> {
		return obj.refresh().then((value: IMetadata) => {
			...
			this.logService.error(`CacheEngine - unable to refresh cache for id: ${obj.item.id}`, e);
