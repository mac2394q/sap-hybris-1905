...
import {
	EditorFieldMappingService, GenericEditorFactory, GenericEditorStackService, GenericEditorTabService, IFetchDataHandler, IGenericEditorConstructor,
	IRestService, IRestServiceFactory, ISharedDataService, LanguageService,
	Payload,
	SeValidationMessageParser, SystemEventService
...
describe('test GenericEditor class', function() {
	...
	it('_displayValidationErrors will add errors messages and localization languages to the field', function() {
		seValidationMessageParser.parse.and.callFake(function(message: any) {
			return {
				message
			};
		});
	...
	it('GIVEN there are errors caused by an external editor WHEN submit is called THEN the editor must raise a GENERIC_EDITOR_UNRELATED_VALIDATION_MESSAGES_EVENT ', function() {
		...
		// Assert
		expect(systemEventService.publishAsync).toHaveBeenCalledWith("UnrelatedValidationMessagesEvent", {messages: failure.data.errors, sourceGenericEditorId: 'someId'});
	});
	...
	it('GIVEN generic editor WHEN addContentChangeEvent is called THEN new callback function is added to the list of callbacks', function() {
		// GIVEN
		const editor = new GenericEditor({
			smarteditComponentType,
			smarteditComponentId,
	...
	it('GIVEN generic editor AND content change callback is added WHEN unregister is called THEN the callback is removed from the list of callbacks', function() {
		// GIVEN
		const editor = new GenericEditor({
			smarteditComponentType,
			smarteditComponentId,
