...
import {DropdownPopulatorPayload, IRestServiceFactory, LanguageService, UriDropdownPopulator} from 'smarteditcommons';
...
describe('uriDropdownPopulator', function() {
	...
	const restServiceForOptions: jasmine.SpyObj<IRestServiceFactory> = jasmine.createSpyObj<IRestServiceFactory>('restServiceFactory', ['get']);
	const $q = promiseHelper.$q();

	const languageService = jasmine.createSpyObj<LanguageService>('languageService', ['getResolveLocale']);
	...
	beforeEach(function() {
		...
		restServiceForOptions.get.and.callFake(function(params: any) {
			const filteredOptions = params ? options.filter(function(option: any) {
		...
		const getDataFromResponse = (response: any) => {
			return response[Object.keys(response).filter(function(key) {
				return response[key] instanceof Array;
			})[0]];
		};

		uriDropdownPopulator = new UriDropdownPopulator(lo, $q, restServiceFactory, getDataFromResponse, getKeyHoldingDataFromResponse, languageService);
	});
