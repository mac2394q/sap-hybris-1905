...
import {scriptUtils, ScriptUtils} from './ScriptUtils';
...
@SeModule({
	...
	providers: [
		 ...
		 *
		 * </pre>
		 * @deprecated since 1905, use {@link functionsModule.service:UrlUtils#getQueryString UrlUtils#getQueryString}
			...
			useFactory: () => {
				...
				return {
					...
					execute(conf: {srcs: string[], callback: SeFactory, index?: number}) {
						return scriptUtils.inject(conf);
					}
				};
			}
		},
		...
		},
		{
			...
			useValue: () => {
				...
				const getClassName = function(instance: any): string | null {
					return instance && instance.constructor && instance.constructor.name && instance.constructor.name !== 'Object' ? instance.constructor.name : null;
				};

				const readObjectStructure = function(json: any, recursiveCount: number = 0) {
					...
					if (typeof json === 'function') {
						...
						if (json.forEach) {
							...
							json.forEach(function(arrayElement: any) {
								recursiveCount++;
								arr.push(readObjectStructure(arrayElement, recursiveCount));
							});
							return arr;
						} else {
							return "JQUERY";
						}
					} else if (json.constructor && json.constructor.name && json.constructor.name !== 'Object') {
						...
						Object.keys(json).forEach(function(directKey) {
							if (directKey.indexOf("$") !== 0) {
								recursiveCount++;
								clone[directKey] = readObjectStructure(json[directKey], recursiveCount);
							}
						});
						return clone;
					}
