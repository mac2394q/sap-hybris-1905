...
describe('cacheEngine', () => {
	...
	const logService: jasmine.SpyObj<LogService> = jasmine.createSpyObj<LogService>('logService', ['error', 'warn']);
	...
	beforeEach(() => {
		...
		spyOn(windowUtils, 'runIntervalOutsideAngular').and.callFake((_callback: () => Promise<void>, interval: number) => {
	...
	it('should not store the same item twice', () => {
		...
		expect(logService.warn).toHaveBeenCalledWith(`CacheEngine - item already exist for id: ${MOCK_ITEM.id}`);
	});
	...
	it('GIVEN an item is handled twice and it\'s cache is not expired, THEN it should return the cache without refreshing it', (done) => {
		const refresh: jasmine.Spy = jasmine.createSpy('refresh').and.returnValue(Promise.resolve(null));
	...
	it('GIVEN a item does not have a cache and is handled twice, THEN it should refresh the item only once AND return the expected cache', (done) => {
		...
		const refresh: jasmine.Spy = jasmine.createSpy('refresh').and.returnValue(Promise.resolve(expectedReturnCache));
	...
	it('GIVEN an item is automatically refreshed, THEN it should log an Error if the refresh action fail', (done) => {
		...
		const refresh: jasmine.Spy = jasmine.createSpy('refresh').and.returnValue(Promise.reject(error));

		cacheEngine.addItem(ANY_MOCK_ITEM, MOCK_DEFAULT_CACHE_TIMING, refresh);

		intervalCallback().then(() => {
			expect(refresh).toHaveBeenCalled();
			expect(logService.error).toHaveBeenCalledWith(`CacheEngine - unable to refresh cache for id: ${ANY_MOCK_ITEM.id}`, `${error}`);
