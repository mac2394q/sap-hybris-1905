...
describe('systemEventService', () => {
	...
	function setPromiseMocks(promise1: Promise<any>, promise2: Promise<any>, promise3: Promise<any>) {
	...
	describe('subscribe', () => {
		it('subscribe event handler with incorrect eventId', () => {
			...
			expect(logService.error).toHaveBeenCalledWith('Failed to subscribe event handler for event: null');
		...
		it('subscribe event handler with incorrect eventId', () => {
			...
			expect(logService.error).toHaveBeenCalledWith('Failed to subscribe event handler for event: ' + id);
			expect(handlerHolderMock.handler).not.toHaveBeenCalledWith(id, data);
		});
	...
	describe('unsubscribe', () => {
		it('unsubscribe event handler with incorrect eventId', () => {
			...
			expect(logService.warn).toHaveBeenCalledWith('Attempting to remove event handler for null but handler not found.');
		});
	});
	...
	describe('publishAsync', () => {
		...
		it('When first handler out of 2 is successful Will call both my event handlers subscribed to the same eventId - with correct id and data - and resolves the promise chain to the resolved data of the last subscriber', (done) => {
			...
			setPromiseMocks(Promise.resolve('firstValue'), Promise.resolve('secondValue'), pendingPromise());
			...
			promise.then((resolvedData: string) => {
	...
	it('When more than 2 handlers and all are successful , resolves the promise chain to the resolved data of the last subscriber', (done) => {
		...
		systemEventService.publish(id, data).then((resolvedData: string) => {
	...
	it('Will NOT call my handler after it has been unsubscribed for the specific eventId and send method promise resolves', () => {

		setPromiseMocks(pendingPromise(), pendingPromise(), pendingPromise());

		const unsubscribeFn = systemEventService.subscribe(id, handlerHolderMock.handler);
		unsubscribeFn();
		systemEventService.publish(id, data).then(angular.noop, () => {
			fail();
		});
