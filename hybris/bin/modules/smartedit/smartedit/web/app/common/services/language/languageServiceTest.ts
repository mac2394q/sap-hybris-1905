...
import {IRestService, IRestServiceFactory} from '../rest/rest';
import {IStorageService} from '../interfaces';
import {ILanguage, IToolingLanguage, LanguageService} from 'smarteditcommons';
...
describe('languageService', () => {
	...
	const translateService: jasmine.SpyObj<TranslateService> = jasmine.createSpyObj<TranslateService>("translateService", ["use", "get"]);
	let gatewayFactory: jasmine.SpyObj<GatewayFactory>;
	let gateway: any;
	let crossFrameEventService: jasmine.SpyObj<CrossFrameEventService>;
	let browserService: any;
	const storageService: jasmine.SpyObj<IStorageService> = jasmine.createSpyObj('storageService', ['getValueFromLocalStorage', 'setValueInLocalStorage']);
	...
	it('GIVEN I select a language THEN it should save the language in the cookie AND switch the language AND publish an event to the gateway', () => {
		...
		expect(storageService.setValueInLocalStorage).toHaveBeenCalledWith(SELECTED_LANGUAGE, language, false);
