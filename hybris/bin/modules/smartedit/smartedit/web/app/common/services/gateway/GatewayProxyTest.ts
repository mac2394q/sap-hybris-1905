...
import {stringUtils, FunctionsUtils, GatewayFactory, GatewayProxy, LogService, MessageGateway, PromiseUtils} from 'smarteditcommons';
...
describe('test gatewayProxy', () => {
	...
	beforeEach(() => {
		...
		gatewayProxy = new GatewayProxy(logService, promiseUtils, stringUtils, functionsUtils, gatewayFactory);
	});
	...
	it('gatewayProxy will proxy empty functions and will wrap the value returned by the proxy in a promise', (done) => {
		...
		// Act
		service.methodToBeProxied('arg1', 'arg2').then((value: any) => {
			expect(gateway.publish).toHaveBeenCalledWith('methodToBeProxied', {
				arguments: ['arg1', 'arg2']
			});
