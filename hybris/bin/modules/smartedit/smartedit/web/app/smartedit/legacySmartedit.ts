...
@SeModule({
	...
	initialize: (
		...
		smartEditContractChangeListener.onComponentsRemoved((components: {component: HTMLElement, parent: HTMLElement, oldAttributes: TypedMap<string>}[], isEconomyMode: boolean) => {
			if (!isEconomyMode) {
				seNamespaceService.reprocessPage();
				renderService._resizeSlots();
			}
			components.forEach((entry) => renderService._destroyComponent(entry.component, entry.parent, entry.oldAttributes));
