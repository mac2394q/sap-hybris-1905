...
class BootstrapService {
	...
	bootstrap(): void {
		...
		const modules = (smarteditNamespace.applications || []).map((application: string) => {
		...
		const constants = {
			...
			whiteListedStorefronts: smarteditNamespace.whiteListedStorefronts
		};
		...
		platformBrowserDynamic().bootstrapModule(window.__smartedit__.SmarteditFactory({modules, constants}))
			.then(() => {
