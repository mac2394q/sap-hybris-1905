...
import {nodeUtils, registerCustomComponents, SeModuleConstructor, SMARTEDIT_COMPONENT_NAME} from 'smarteditcommons';
...
export class SmarteditComponent {
	...
	constructor(private elementRef: ElementRef, public upgrade: UpgradeModule, injector: Injector) {
	...
	ngAfterViewInit() {
		if (!nodeUtils.hasLegacyAngularJSBootsrap()) {
			...
			this.upgrade.bootstrap(this.elementRef.nativeElement, [(LegacySmartedit as SeModuleConstructor).moduleName], {strictDi: false});
		}
	}
}
