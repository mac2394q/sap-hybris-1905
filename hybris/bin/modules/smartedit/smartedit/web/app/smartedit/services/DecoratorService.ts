...
import {IDecoratorDisplayCondition, PromiseUtils, SeDowngradeService, StringUtils, TypedMap} from 'smarteditcommons';
 ...
 * 
 */
const DecoratorServiceTOKEN = 'decoratorService';

@SeDowngradeService(DecoratorServiceTOKEN)
@Injectable()
export class DecoratorService {

	public static TOKEN = DecoratorServiceTOKEN;
	...
	constructor(
		...
		private legacyDecoratorToCustomElementConverter: LegacyDecoratorToCustomElementConverter
	) {
	}
	...
	getDecoratorsForComponent(componentType: string, componentId?: string): Promise<string[]> {
		...
		decoratorArray.forEach((dec: string) => {
			...
			if (activeDecorator && activeDecorator.displayCondition) {
				...
				activeDecorator.displayCondition(componentType, componentId).then((display: boolean) => {
					if (display) {
		...
		return Promise.all(promisesToResolve).then((decoratorsEnabled) => {
			return displayedDecorators.concat(decoratorsEnabled.filter((dec) => {
				return dec;
			}));
		});
	}
