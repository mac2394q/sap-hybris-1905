...
export class SmartEditContractChangeListener {
	...
	*/
	...
	setEconomyMode(_mode: boolean): void {
		...
		if (!this.economyMode) {
			// reactivate
			Array.prototype.slice.apply(this.componentHandlerService.getFirstSmartEditComponentChildren(this.yjQuery('body'))).forEach((firstLevelComponent: HTMLElement) => {
	...
	private wasSmarteditNode(node: HTMLElement): boolean {
		return this.componentsQueue.some((componentInQueue) => {
			return componentInQueue.component === node;
		}) && this.smartEditAttributeNames.every((attributeName) => {
			return !node.hasAttribute(attributeName);
		});
	}
	...
	private hasSmarteditOldAttributeChanged(entry: TargetedNode): boolean {
		return this.lodash.keys(entry.oldAttributes).some((attributeName) => {
			return this.smartEditAttributeNames.indexOf(attributeName) > -1;
		});
	}
			...
			this.aggregateMutationsOnChangedAttributes(mutations).forEach((entry: TargetedNode) => {
				if (this.hasSmarteditOldAttributeChanged(entry)) {
					if (this.isSmarteditNode(entry.node)) {
						const componentIndex = this._getComponentIndexInQueue(entry.node as HTMLElement);
						if (componentIndex === -1) {
