...
export class ComponentHandlerService {
	...
	getFirstSmartEditComponentChildren(htmlElement: HTMLElement | JQuery): JQuery[] {
		const stem: any = this.yjQuery(htmlElement); // using any as getCssPath is a yJquery augmentation
		...
		return stem.find("." + this.COMPONENT_CLASS).filter((index: number, component: JQuery) => {
			const match: string = (this.yjQuery(component) as any).getCssPath().replace(parentCssPath, "").match(firstChildrenRegex);
