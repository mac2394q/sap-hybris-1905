...
import {registerCustomComponents, SeModuleConstructor, SMARTEDITCONTAINER_COMPONENT_NAME, SMARTEDITLOADER_COMPONENT_NAME} from "smarteditcommons";
...
@Component({
	...
	template: `
		...
		<div id='${legacyAppName}'>
			<y-announcement-board></y-announcement-board>
...
export class SmarteditcontainerComponent {
	...
	constructor(private translateService: TranslateService, injector: Injector, public upgrade: UpgradeModule, private elementRef: ElementRef) {
	...
	ngOnInit() {
		...
		Array.prototype.slice.call(document.body.childNodes)
			.filter((childNode: ChildNode) => !this.isAppComponent(childNode) && !this.isSmarteditLoader(childNode))
	...
	ngAfterViewInit() {
		this.upgrade.bootstrap(this.legacyAppNode, [(Smarteditcontainer as SeModuleConstructor).moduleName], {strictDi: false});
	}
	...
	private isAppComponent(childNode: ChildNode) {
		return childNode.nodeType === Node.ELEMENT_NODE && (childNode as HTMLElement).tagName === SMARTEDITCONTAINER_COMPONENT_NAME.toUpperCase();
	}

	private isSmarteditLoader(childNode: ChildNode) {
		return childNode.nodeType === Node.ELEMENT_NODE &&
			((childNode as HTMLElement).id === 'smarteditloader' || (childNode as HTMLElement).tagName === SMARTEDITLOADER_COMPONENT_NAME.toUpperCase());
	}
}
