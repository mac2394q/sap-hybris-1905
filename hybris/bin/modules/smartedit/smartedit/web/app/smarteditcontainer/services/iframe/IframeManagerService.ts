 ...
 */
import {IConfiguration, ISharedDataService, SeInjectable, TypedMap} from 'smarteditcommons';
...
export class IframeManagerService {
	...
	load(url: string, checkIfFailingHTML?: boolean, pageInPreviewMode?: string): void {
		if (checkIfFailingHTML) {
			this._getPageAsync(url).then(() => {
				...
				this.$log.error(`IFrameManagerService.load - _getPageAsync failed with error ${error}`);
			});
		} else {
	...
	loadPreview(homePageOrPageFromPageList: string, previewTicket: string): void {
		...
		if (!/.+\.html/.test(homePageOrPageFromPageList)) { // for testing purposes
			promiseToResolve = this._appendURISuffix(homePageOrPageFromPageList);
		} else {
			promiseToResolve = this.$q.when(homePageOrPageFromPageList);
		}

		promiseToResolve.then((previewURL: string) => {
    ...
    */
	...
	private _appendURISuffix(url: string): angular.IPromise<string> {
		const pair = url.split('?');
		return this.sharedDataService.get('configuration').then((configuration: IConfiguration) => {
			if (!configuration || !configuration.storefrontPreviewRoute) {
				this.$log.debug("SmartEdit configuration for 'storefrontPreviewRoute' is not found. Fallback to default value: '" + IframeManagerService.DEFAULT_PREVIEW_ROUTE + "'");
			...
			return pair[0]
				...
				.replace(/(.+)\/$/g, "$1/" + previewRoute) + (pair.length === 2 ? "?" + pair[1] : "");
		});
	}
