...
export class StorageService extends IStorageService {
	...
	getAuthTokens(): ISessionAuth {
		const smarteditSessions = this.windowUtils.getWindow().localStorage.getItem(this.SMARTEDIT_SESSIONS);
	...
	setValueInLocalStorage(cookieName: string, value: any, encode: boolean) {
		this._setValueInLocalStorage(cookieName, value, encode);
	}
	...
	private _setSmarteditSessions(sessions: ISessionAuth) {
		...
		this.windowUtils.getWindow().localStorage.setItem(this.SMARTEDIT_SESSIONS, sessionsEncrypted);
	}
