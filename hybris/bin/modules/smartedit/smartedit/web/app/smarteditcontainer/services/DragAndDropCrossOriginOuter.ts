...
import {CrossFrameEventService, IDragAndDropEvents, IMousePosition, SeInjectable, TypedMap} from "smarteditcommons";
...
export class DragAndDropCrossOrigin {
	...
	constructor(
		...
		private DRAG_AND_DROP_CROSS_ORIGIN_BEFORE_TIME: TypedMap<string>
	) {}
	...
	initialize(): void {
		...
		this.crossFrameEventService.subscribe(this.DRAG_AND_DROP_CROSS_ORIGIN_BEFORE_TIME.START, () => {
			if (this.isEnabled()) {
				this.iframeManagerService.getIframe().css('pointer-events', 'none');
			}
		});
		this.crossFrameEventService.subscribe(this.DRAG_AND_DROP_CROSS_ORIGIN_BEFORE_TIME.END, () => {
			if (this.isEnabled()) {
