...
export default abstract class WhitelistingUtils {
	...
	static readonly whitelistingConfigRegex = new RegExp(/^(([-*a-zA-Z0-9]+[.])*([-a-zA-Z0-9]+[.]))?[-a-zA-Z0-9]+(:[0-9]{1,5})$/);
	...
	static convertWhitelistingToRegexp(whitelistings: string[]): RegExp[] {
		...
		return whitelistings.map((origins) => {
			...
			if (WhitelistingUtils.whitelistingConfigRegex.test(trimmed)) {
				const regexpKey = ['^', '$'].join(trimmed.replace(/\./g, '\\.').replace(/\*/g, '[-a-zA-Z0-9]*'));
     ...
     */
	static getWhitelistFromScriptElement(scriptElement: HTMLScriptElement, global: Window): string[] {
		...
		const attributeList = scriptElement.getAttribute(WhitelistingUtils.allowOriginAttributeName) || "";
		if (attributeList) {
		...
		const encodedAllowOrigin = WebUtils.extractQueryParameter(url.search, WhitelistingUtils.allowOriginQueryParamName);
	...
	static getSanitizedHostFromLocation(location: HTMLHyperlinkElementUtils | Location) {
		...
		const port = location.port || (location.protocol.replace(/:/g, '') === 'https' ? '443' : '80');
