...
export default abstract class WebUtils {
	...
	static getWebappScriptElementFromDocument(document: Document): HTMLScriptElement {
		...
		const scriptElement = document.querySelector<HTMLScriptElement>(`script#${WebUtils.webappScriptId}`);
		if (scriptElement) {
			return scriptElement;
		}

		// This will be the case for IE (if no smartedit-injector attr used)
		const nodeListElements: NodeListOf<HTMLScriptElement> = document.querySelectorAll(`script[src*=${WebUtils.webappScriptName}]`);
		if (nodeListElements.length !== 1) {
			throw new Error(`SmartEdit unable to load - invalid ${WebUtils.webappScriptName} script tag`);
		}
	...
	static extractQueryParameter(url: string, queryParameterName: string): string {
		...
		url.replace(/([?&])([^&=]+)=([^&]*)?/g, ($0: any, $1: any, parameterKey: any, parameterValue: any) => {
