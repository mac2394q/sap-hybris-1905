...
module.exports = (grunt) => {
    ...
    grunt.registerTask('prepareJsTarget_Dev', 'prepareJsTarget_Dev', () => {
        const webpackCommonsTasks = ['webpack:libSmarteditCommons', 'shell:internalSmarteditCommons', 'webpack:devSmarteditCommons'];
        let webpackTasks = [];
    ...
    // -------------------------------------------------------------------------------------------------
    // TEST
    grunt.registerTask('unit', ['karma:unitSmarteditcommons', 'karma:unitSmartedit', 'karma:unitSmarteditContainer']);
    grunt.registerTask('test_only', ['generate', 'unit']); // Legacy - see yunit macro in buildcallbacks.xml
    grunt.registerTask('e2e', ['buildE2EScripts', 'connect:dummystorefront', 'connect:test', 'protractorRun']);
    grunt.registerTask('e2e_max', ['buildE2EScripts', 'connect:dummystorefront', 'connect:test', 'protractorMaxrun']);
    grunt.registerTask('e2e_dev', 'e2e local development mode', () => {
        ...
        grunt.task.run(['generateE2eListHtml', 'buildE2EScripts', 'connect:test', 'connect:dummystorefront']);
    });
    ...
    // -------------------------------------------------------------------------------------------------
    // PREPARE STYLING
    grunt.registerTask('styling_dev', [
        'webfont',
        'less',
    ]);
    ...
    grunt.registerTask('prod', ['formatCode', 'clean:lib', 'prepareJsTarget_Prod', 'prepareWebroot_Prod']);
    ...
    grunt.registerTask('dev', 'dev', () => {
        const tasks = ['formatCode', 'prepareJsTarget_Dev', 'prepareWebroot_Dev', 'declareTypes'];
        if (!grunt.option('target') || /^(commons)$/.test(grunt.option('target'))) {
            tasks.unshift('clean:lib');
        }
        grunt.task.run(tasks);
        grunt.log.writeln('To speed up build time, you can run:\n`grunt dev --target=inner`\n`grunt dev --target=outer`\n`grunt dev --target=inner,outer`\n`grunt dev --target=inner,commons`\netc...');
    });
