...
describe('smartEditContractChangeListener in polyfill mode', () => {
	...
	describe('DOM mutations', () => {
		...
		it('should add a component to the components queue if it was generated from a simple div by adding smartedit attributes (change operation)', async () => {
			// GIVEN
			(smartEditContractChangeListener as any).componentsQueue = [];
			const mutations: any = [{
				type: 'attributes',
				attributeName: UUID_ATTRIBUTE,
				target: component1,
			...
			// THEN
			const componentsInQueue = (smartEditContractChangeListener as any).componentsQueue.map((cmp: any) => cmp.component);
		...
		it('should remove component from the queue if smartedit attributes were removed from it (change operation)', async () => {
			...
			let componentsInQueue = (smartEditContractChangeListener as any).componentsQueue.map((cmp: any) => cmp.component);
			expect(componentsInQueue).toContain(component1);
			...
			// THEN
			componentsInQueue = (smartEditContractChangeListener as any).componentsQueue.map((cmp: any) => cmp.component);
