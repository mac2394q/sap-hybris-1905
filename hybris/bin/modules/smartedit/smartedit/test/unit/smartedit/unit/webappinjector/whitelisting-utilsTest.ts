...
describe('Whitelisting-Utils from webapp-injector', () => {
	...
	it('getWhitelistFromScriptElement should include all allow origin attribute values', () => {
		...
		const scriptElement = document.createElement('script');
		scriptElement.setAttribute(WhitelistingUtils.allowOriginAttributeName, allowedConfigs.join(','));
		expect(WhitelistingUtils.getWhitelistFromScriptElement(scriptElement, window)).toEqual(whitelistingRegexes);
	});
	...
	it('getWhitelistFromScriptElement should include all allow origin query param values', () => {
		...
		const scriptElement = document.createElement('script');
		scriptElement.src =
			`webApplicationInjector.js?${WhitelistingUtils.allowOriginQueryParamName}=${allowedConfigs.join(',')}`;
		expect(WhitelistingUtils.getWhitelistFromScriptElement(scriptElement, window)).toEqual(whitelistingRegexes);
	});
	...
	it('getWhitelistFromScriptElement should include both attribute AND query param allow origins', () => {
		...
		const scriptElement = document.createElement('script');
		scriptElement.setAttribute(WhitelistingUtils.allowOriginAttributeName, firstHalfOfWhitelistingRegexes.join(','));
		scriptElement.src =
			`webApplicationInjector.js?${WhitelistingUtils.allowOriginQueryParamName}=${secondHalfOfWhitelistingRegexes.join(',')}`;
		expect(WhitelistingUtils.getWhitelistFromScriptElement(scriptElement, window)).toEqual(whitelistingRegexes);
	});
