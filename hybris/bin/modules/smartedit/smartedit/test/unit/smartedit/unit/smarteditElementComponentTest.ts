...
describe("SmarteditElementComponent Test", () => {
	...
	let upgradeModule: jasmine.SpyObj<UpgradeModule>;
	...
	const SMARTEDIT_DRAG_AND_DROP_EVENTS = {
		DRAG_DROP_START: 'DRAG_DROP_START',
		DRAG_DROP_END: 'DRAG_DROP_END'
	};
	...
	let dNDStartCallback: (eventId: string, smarteditComponentClosestToDraggedElement?: angular.IAugmentedJQuery) => void;
	...
	const ORIGINAL_ELEMENT =
		`<smartedit-element data-smartedit-element-uuid="theSmarteditElementUuid">
	...
	beforeEach(() => {
		...
		unregisterPerspectiveChangeEventSpy = spyOn(unregisterersMap, 'unregisterPerspectiveChangeEvent');
		unregisterPerspectiveRefreshedEventSpy = spyOn(unregisterersMap, 'unregisterPerspectiveRefreshedEvent');
		...
		smarteditComponentClosestToDraggedElement = jasmine.createSpyObj<angular.IAugmentedJQuery>("smarteditComponentClosestToDraggedElement", ['attr']);
		sakExecutorService = jasmine.createSpyObj<SakExecutorService>("sakExecutorService", ['wrapDecorators']);

		outputFromWrappedDecorators = document.createElement("outputFromWrappedDecorators");

		sakExecutorService.wrapDecorators.and.callFake((projectedContent: string, element: HTMLElement) => {
			if (projectedContent.trim() === '<some-projected-content>someProjectedContent</some-projected-content>'
				&& element.tagName === 'SMARTEDIT-ELEMENT') {
				outputFromWrappedDecorators.setAttribute('active', 'false');
				outputFromWrappedDecorators.setAttribute("data-smartedit-element-uuid", "theSmarteditElementUuid");
				return $q.when(outputFromWrappedDecorators);
			} else {
				throw new Error(`unexpected parameters sent to sakExecutorService.wrapDecorators: projectedContent ${projectedContent} and element.tagName: ${element.tagName}`);
			}
		});

		crossFrameEventService = jasmine.createSpyObj<CrossFrameEventService>("crossFrameEventService", ['subscribe']);
		crossFrameEventService.subscribe.and.callFake((eventId: string, callback: (eventId: string, notPreview: boolean) => void) => {
			if (eventId === EVENT_PERSPECTIVE_CHANGED) {
				...
				throw new Error(`unexpected eventId ${eventId} passed to crossFrameEventService.subscribe`);
			}
		});

		systemEventService = jasmine.createSpyObj<SystemEventService>("systemEventService", ['subscribe']);
		systemEventService.subscribe.and.callFake((eventId: string, callback: (eventId: string, smarteditComponentClosestToDraggedElement?: angular.IAugmentedJQuery) => void) => {
			if (eventId === SMARTEDIT_DRAG_AND_DROP_EVENTS.DRAG_DROP_START) {
				...
				throw new Error(`unexpected eventId ${eventId} passed to systemEventService.subscribe`);
			}
		});

		polyfillService = jasmine.createSpyObj<PolyfillService>("polyfillService", ['isEligibleForEconomyMode']);
		...
		upgradeModule = jasmine.createSpyObj<UpgradeModule>("upgradeModule", ['bootstrap']);
		injector = jasmine.createSpyObj<Injector>("injector", ['get']);
		(upgradeModule as any).injector = injector;
		...
		smarteditElementComponent = new SmarteditElementComponent(upgradeModule, elementRef, sakExecutorService, systemEventService, crossFrameEventService);
	...
	it("event listeners are registered", () => {
		...
		expect(crossFrameEventService.subscribe.calls.argsFor(0)[0]).toBe(EVENT_PERSPECTIVE_CHANGED);
		expect(crossFrameEventService.subscribe.calls.argsFor(1)[0]).toBe(EVENT_PERSPECTIVE_REFRESHED);
		expect(crossFrameEventService.subscribe.calls.argsFor(2)[0]).toBe(EVENT_SMARTEDIT_COMPONENT_UPDATED);

		expect(systemEventService.subscribe.calls.count()).toBe(2);
		expect(systemEventService.subscribe.calls.argsFor(0)[0]).toBe(SMARTEDIT_DRAG_AND_DROP_EVENTS.DRAG_DROP_START);
		expect(systemEventService.subscribe.calls.argsFor(1)[0]).toBe(SMARTEDIT_DRAG_AND_DROP_EVENTS.DRAG_DROP_END);
	...
	it("on init, projected content is removed, fed to sakExecutor that returns a 'non active' node appended to the native element", () => {
		...
		expect(stringUtils.formatHTML(nativeElement.outerHTML)).toBe(stringUtils.formatHTML(PROCESSED_ELEMENT(false)));
	...
	it("on destroy, native element returns to initial state (not a real case but easier to test removal of decorators)", () => {
		...
		expect(stringUtils.formatHTML(nativeElement.outerHTML)).toBe(stringUtils.formatHTML(ORIGINAL_ELEMENT));
	});
	...
	it("in economy mode, decorators are removed from dragged element when drag and drop starts", () => {
		...
		dNDStartCallback(SMARTEDIT_DRAG_AND_DROP_EVENTS.DRAG_DROP_START, smarteditComponentClosestToDraggedElement);
	...
	it("on mouseenter if not active and decorators are enabled, decorators are set active", () => {
		...
		smarteditElementComponent.active = false;
		(smarteditElementComponent as any).componentDecoratorEnabled =
			mouseenterListenerCallback();

		expect(sakExecutorService.wrapDecorators).not.toHaveBeenCalled();

		expect(stringUtils.formatHTML(nativeElement.outerHTML)).toBe(stringUtils.formatHTML(PROCESSED_ELEMENT(true)));
	...
	function setElementUuidOfDraggedElement(value: string) {
		...
		smarteditComponentClosestToDraggedElement.attr.and.callFake((name: string) => {
			if (name === ELEMENT_UUID_ATTRIBUTE) {
				...
				throw new Error(`unexpected attribute name ${name} passed to smarteditComponentClosestToDraggedElement.attr`);
			}
		});
	}
});