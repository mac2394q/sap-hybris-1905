 ...
 */
import {IDecoratorDisplayCondition, PromiseUtils, StringUtils, TypedMap} from "smarteditcommons";
...
describe('decoratorService', () => {
	...
	let legacyDecoratorToCustomElementConverter: jasmine.SpyObj<LegacyDecoratorToCustomElementConverter>;
	let anyDecoratorService: {_activeDecorators: TypedMap<{displayCondition?: IDecoratorDisplayCondition}>};

	beforeEach(() => {

		promiseUtils = new PromiseUtils();
		stringUtils = new StringUtils();
		legacyDecoratorToCustomElementConverter = jasmine.createSpyObj<LegacyDecoratorToCustomElementConverter>("legacyDecoratorToCustomElementConverter", ['convertIfNeeded']);
	...
	async function setupAsyncOnGetDecoratorsForComponent(componentType: string, componentId?: string) {
	...
	it('getDecoratorsForComponent will retain a unique set of decorators from all matching regexps', (done) => {
		...
		promise.then((decorators) => {
			expect(decorators).toEqual(['decorator1', 'decorator2', 'decorator3', 'decorator4', 'decorator5']);

			promise = setupAsyncOnGetDecoratorsForComponent('TypeSuffixes');
			promise.then((decorators2) => {
				expect(decorators2).toEqual(['decorator2', 'decorator3', 'decorator4', 'decorator5']);
