...
describe('Whitelisting-Utils from webapp-injector', () => {
	...
	describe('getWebappScriptElementFromDocument()', () => {
		it('Throw error when currentScript exists but is invalid', () => {
			...
			expect(() => WebUtils.getWebappScriptElementFromDocument(mockDocument as Document)).toThrowError();
		});
		...
		it('Return valid currentScript element', () => {
			...
			expect(WebUtils.getWebappScriptElementFromDocument(mockDocument as Document)).toBe(validScritpElement);
		});
		...
		it('Returns script element from element id', () => {
			...
			expect(WebUtils.getWebappScriptElementFromDocument(mockDocument as Document)).toBe(mock);
		});
		...
		it('Throw error when not exactly 1 webApplicationInjector found by script src', () => {
			...
			expect(() => WebUtils.getWebappScriptElementFromDocument(mockDocument as unknown as Document)).toThrowError();
		});
		...
		it('Returns 1 webApplicationInjector script when found by script src', () => {
			...
			expect(WebUtils.getWebappScriptElementFromDocument(mockDocument as Document)).toBe(dummy as any);
		});
	});
