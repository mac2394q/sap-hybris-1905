...
describe('LegacyDecoratorToCustomElementConverter test', () => {
	...
	beforeEach(() => {
		...
		customElements.define('other-custom-element', class extends HTMLElement {
			...
			connectedCallback(): void {
				...
				if (this.isConnected) {
					...
					Array.prototype.slice.call(this.childNodes).forEach((childNode: ChildNode) => {
			...
			disconnectedCallback(): void {
				if (!this.isConnected && this.content) {
					Array.prototype.slice.call(this.content.childNodes).forEach((childNode: ChildNode) => {
	...
	it(`both decorators are compiled sequentially 
        ...
        <deco-2 class='deco2 se-decorator-wrap'
			...
			const outerHTML = await promiseUtils.resolveToCallbackWhenCondition(
				() => !!document.querySelector("sometemplatecontent2") && !!document.querySelector("sometemplatecontent1") && !!document.querySelector("#someProjectedContent"),
			...
			expect(converter.getScopes()).toEqual(['thesmarteditComponentId_DECO-2', 'thesmarteditComponentId_DECO-1']);
		});
