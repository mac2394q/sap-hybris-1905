...
describe('iframeManagerService', () => {
	...
	const sharedDataService: jasmine.SpyObj<ISharedDataService> = jasmine.createSpyObj('sharedDataService', ['get']);
	...
	it('GIVEN that _mustLoadHasSuch has returned false WHEN I request to load a preview THEN the page will be first loaded in preview mode, then we will load the currentLocation', () => {
		...
		// Assert
		expect($http.get).toHaveBeenCalledWith('myurl/testPreviewRoute?cmsTicketId=previewTicket1');
	...
	it('iframeManagerService loadPreview appends storefront preview route suffix to the url and the preview ticket to the query string case 1', () => {
		// Arrange
		spyOn(iframeManagerService, 'load');
		...
		// Assert
		expect(iframeManagerService.load).toHaveBeenCalledWith('myurl/testPreviewRoute?cmsTicketId=previewTicket1');
	});
	it('iframeManagerService loadPreview appends storefront preview route suffix to the url and the preview ticket to the query string case 2', () => {
		// Arrange
		spyOn(iframeManagerService, 'load');
	...
	it('iframeManagerService loadPreview appends storefront preview route suffix to the url and the preview ticket to the query string case 3', () => {
		// Arrange
		spyOn(iframeManagerService, 'load');
	...
	it('iframeManagerService loadPreview appends storefront preview route suffix to the url and the preview ticket to the query string case 4', () => {
		// Arrange
		spyOn(iframeManagerService, 'load');
	...
	it('iframeManagerService loadPreview, without any configuration, appends default storefront preview route suffix to the url and the preview ticket to the query string case 5', () => {
		// Arrange
		spyOn(iframeManagerService, 'load');
		sharedDataService.get.and.callFake(function() {
			return $q.when({
	...
	it('iframeManagerService loadPreview, without storefrontPreviewRoute configuration, appends default storefront preview route suffix to the url and the preview ticket to the query string case 6', () => {
		// Arrange
		spyOn(iframeManagerService, 'load');
		sharedDataService.get.and.callFake(function() {
			return $q.when({
