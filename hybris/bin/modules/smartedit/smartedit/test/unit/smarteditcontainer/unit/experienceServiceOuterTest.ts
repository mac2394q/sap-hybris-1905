...
describe('experienceService', function() {
	...
	beforeEach(() => {
		...
		const propsService = new StoragePropertiesService([], lo);

		experienceService = new ExperienceService($q, $location, $log, $route, lo, crossFrameEventService, siteService, catalogService, languageService,
	...
	it('checks GatewayProxied', () => {
		...
		expect(decoratorObj).toEqual(['loadExperience', 'updateExperiencePageContext', 'getCurrentExperience', 'hasCatalogVersionChanged', 'buildRefreshedPreviewUrl', 'compareWithCurrentExperience']);
	});
	...
	it('WHEN compareWithCurrentExperience is called with no experience THEN it returns promise with false', function() {

		// WHEN
		const promise = experienceService.compareWithCurrentExperience(null) as IExtensiblePromise<boolean>;
	...
	it('WHEN compareWithCurrentExperience is called with experience same as current experience THEN it returns promise with true', function() {

		// WHEN
		const promise = experienceService.compareWithCurrentExperience({
			pageId: 'oldPageId',
			catalogId: 'someCatalog',
			catalogVersion: 'someVersion',
			siteId: 'someSite'
	...
	it('WHEN compareWithCurrentExperience is called with experience which is different than current experience THEN it returns promise with false', function() {
