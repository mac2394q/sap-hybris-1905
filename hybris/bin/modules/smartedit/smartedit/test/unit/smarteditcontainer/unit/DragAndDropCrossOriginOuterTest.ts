...
describe('DragAndDropCrossOriginOuter', () => {
	...
	beforeEach(() => {
		...
		spyOn(lo, 'throttle').and.returnValue(throttledSendMousePosition);
	...
	it('initialize will subscribe to 4 events', () => {
		...
		expect(crossFrameEventService.subscribe).toHaveBeenCalledWith(DRAG_AND_DROP_CROSS_ORIGIN_BEFORE_TIME.START, jasmine.any(Function));
		expect(crossFrameEventService.subscribe).toHaveBeenCalledWith(DRAG_AND_DROP_CROSS_ORIGIN_BEFORE_TIME.END, jasmine.any(Function));
	});
