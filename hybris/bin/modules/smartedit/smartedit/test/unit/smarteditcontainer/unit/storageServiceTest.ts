...
import {annotationService, FingerPrintingService, GatewayProxied, IAuthToken, LogService, WindowUtils} from 'smarteditcommons';
import {StorageService} from 'smarteditcontainer/services';

describe('outer storage service', () => {

	const cryptographicUtils: jasmine.SpyObj<any> = jasmine.createSpyObj('cryptographicUtils', ['aesBase64Encrypt', 'aesDecrypt']);
	const fingerPrintingService: jasmine.SpyObj<FingerPrintingService> = jasmine.createSpyObj('fingerPrintingService', ['getFingerprint']);
	const windowUtils: jasmine.SpyObj<WindowUtils> = jasmine.createSpyObj('windowUtils', ['getWindow']);
	let store: any = {};
	const $window = {
		location: {
	...
	beforeEach(() => {
		...
		storageService = new StorageService(new LogService(), windowUtils, cryptographicUtils, fingerPrintingService);
	...
	it('checks GatewayProxied', () => {
		...
		expect(decoratorObj).toEqual(['isInitialized', 'storeAuthToken', 'getAuthToken', 'removeAuthToken', 'removeAllAuthTokens', 'storePrincipalIdentifier', 'getPrincipalIdentifier', 'removePrincipalIdentifier', 'getValueFromCookie', 'getValueFromLocalStorage']);
	});
	...
	it('WHEN setValueInLocalStorage is called and the encode flag is not set THEN the un-encoded value is stored', () => {
		...
		// Assert
		expect($window.localStorage.setItem).toHaveBeenCalledWith('someCookie', JSON.stringify(rawValue));
	});
