...
describe('configurationExtractorService', () => {
	...
	describe('extractSEContainerModules', () => {
		...
		beforeEach(() => {
			...
			result = new ConfigurationExtractorService($log, lo).extractSEContainerModules(CONFIGURATION);
		});
	...
	describe('extractSEModules', () => {
		...
		beforeEach(() => {
			result = new ConfigurationExtractorService($log, lo).extractSEModules(CONFIGURATION);
		});
	...
	describe('orderApplications', () => {
		...
		it('applications will be sorted by means of extends keyword', () => {
			...
			$log = jasmine.createSpyObj<angular.ILogService>(" $log", ['error']);
			const service = new ConfigurationExtractorService($log, lo);
			...
			expect($log.error).toHaveBeenCalledWith("Application AppE located at SEContainerLocationForAppE is ignored because it extends an unknown application 'unkownApp'; SmartEdit functionality may be compromised.");
