...
describe('outer perspectiveService', () => {
	...
	const storageService: jasmine.SpyObj<StorageService> = jasmine.createSpyObj('storageService', ['getValueFromLocalStorage', 'setValueInLocalStorage']);
	...
	const EVENT_PERSPECTIVE_UPDATED: string = 'EVENT_PERSPECTIVE_UPDATED';
	...
	const PERSPECTIVE_SELECTOR_WIDGET_KEY: string = 'perspectiveToolbar.perspectiveSelectorTemplate';
	...
	beforeEach(() => {
		...
		perspectiveService = new PerspectiveService($rootScope, $location, STORE_FRONT_CONTEXT, $log, $q, isBlank, uniqueArray, systemEventService,
			...
			EVENT_PERSPECTIVE_REFRESHED, PERSPECTIVE_SELECTOR_WIDGET_KEY, EVENT_PERSPECTIVE_UPDATED, permissionService);
	});
	...
	describe('register', () => {
		...
		it('should send EVENT_PERSPECTIVE_ADDED event if register is called the first time and EVENT_PERSPECTIVE_UPDATED is register is called the second time with the same key', () => {
			// WHEN
			perspectiveService.register({
				key: 'somekey',
				nameI18nKey: 'somenameI18nKey',
	...
	describe('_fetchAllFeatures', () => {
		it('collects all features of nested perspectives, returns a set of unique features, and log message if a nested perspective does not exist', () => {
			...
			// THEN
			expect(holder).toEqual([PERSPECTIVE_SELECTOR_WIDGET_KEY, 'feat1', 'feat2', 'feat3', 'feat4']);
	...
	describe('refreshPerspective', () => {
		it('should select the default perspective if there is no active perspective', (done) => {
			...
			storageService.getValueFromLocalStorage.and.returnValue($q.when(null));
			...
			// THEN
			expect(storageService.getValueFromLocalStorage).toHaveBeenCalledWith('smartedit-perspectives', true);
			result.then(() => {
	...
	describe('refreshPerspective - not permitted perspective', () => {
		it('WILL select NONE_PERSPECTIVE if the active perspective is not permitted', (done) => {
	...
	describe('switchTo', () => {
		...
		it('WILL silently do nothing if trying to switch to the same perspecitve as the activated one', () => {
			// GIVEN
			perspectiveService.register({
				key: 'aperspective',
			...
			expect(featureService.enable.calls.argsFor(0)).toEqual([PERSPECTIVE_SELECTOR_WIDGET_KEY]);
		...
		it('WILL activate all (nested) features of a perspective and notify to rerender', () => {
			...
			// THEN
			expect(storageService.setValueInLocalStorage).toHaveBeenCalledWith('smartedit-perspectives', 'persp1', true);
			expect(waitDialogService.showWaitModal).toHaveBeenCalled();
	...
	describe('selectDefault', () => {
		...
		it('WILL select the perspective by disabling all features first if a perspective already exists', () => {
			...
			spyOn(perspectiveService, 'switchTo').and.returnValue($q.when());
			spyOn((perspectiveService as any), '_disableAllFeaturesForPerspective');
			storageService.getValueFromLocalStorage.and.returnValue($q.when(ALL_PERSPECTIVE));
		...
		it('WILL select NONE_PERSPECTIVE if perspective stored in cookie has no permission', () => {
			...
			spyOn(perspectiveService, 'switchTo').and.returnValue($q.when());
			spyOn((perspectiveService as any), '_disableAllFeaturesForPerspective');
			storageService.getValueFromLocalStorage.and.returnValue($q.when(PERSPECTIVE_WITHOUT_PAERMISSIONS.key));
