...
module.exports = (function() {
    ...
    componentObject.elements = {
        ...
        getTabSelector: function(tabname, inError) {
            return by.cssContainingText('.se-generic-editor__tabs > div > ul.nav-tabs > li > a' + (inError === true ? '.sm-tab-error' : ''), tabname);
        },
        ...
        getValidationErrors: function(fieldName, language) {
            var fieldStructure = this.getFieldStructure(fieldName);
            if (language) {
                return fieldStructure.all(by.css('y-tab[data-tab-id="' + language + '"] se-generic-editor-field-messages .se-help-block--has-error'));
            } else {
