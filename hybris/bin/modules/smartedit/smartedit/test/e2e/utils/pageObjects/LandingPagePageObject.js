...
module.exports = (function() {
    ...
    componentObject.elements = {
        ...
        getSiteSelectorSearchField: function() {
    ...
    componentObject.actions = {
        ...
        openAndBeReady: function(done) {
            browser.get(componentObject.constants.LANDING_PAGE_PATH).then(function() {
        ...
        navigateToLandingPage: function() {
            return browser.click(element(by.css('sites-link a'))).then(function() {
                browser.waitForContainerToBeReady();
                browser.waitForUrlToMatch(/^(?!.*storefront)/);
            }.bind(this));
        }
    ...
    componentObject.assertions = {
        ...
        assertListOfOptionsInSitestDrodpwn: function(expectedOptions) {
            var dropdownOptionCssSelector = 'y-select[data-id="site"] .ui-select-choices-row-inner';
            ...
            }, 5000, 'Expected dropdown options for site to be ' + expectedOptions);
        },

        searchAndAssertInDropdown: function(searchTerm, expectedOptions) {
            browser.waitForPresence(componentObject.elements.getSiteSelectorSearchField());
            componentObject.elements.getSiteSelectorSearchField().clear().sendKeys(searchTerm);
            componentObject.assertions.assertListOfOptionsInSitestDrodpwn(expectedOptions);
        },
    };
