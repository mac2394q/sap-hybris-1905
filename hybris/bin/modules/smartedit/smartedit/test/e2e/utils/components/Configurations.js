...
module.exports = {
    ...
    getConfigurationsAsList: function() {
        browser.wait(function() {
            return element.all(by.css('#editConfigurationsBody .se-config__entry')).count().then(function(count) {
                return count > 0;
            });
    ...
    clickSave: function() {
        browser.click(element(by.cssContainingText('.modal-dialog .se-modal__footer button', 'Save')));
    },

    clickCancel: function() {
        return browser.click(element(by.css('.modal-dialog .se-modal__footer button#cancel')));
    },

    getCancelButton: function() {
        return element(by.cssContainingText('.modal-dialog .se-modal__footer button', 'Cancel'));
    },

    getSaveButton: function() {
        return element(by.cssContainingText('.modal-dialog .se-modal__footer button', 'Save'));
    },
    ...
    getConfigurationCenterButton: function() {
        return element(by.id('smartEditHeaderToolbar_option_headerToolbar.configurationTemplate_btn'));
    },

    getConfigurations: function() {
        var deferred = protractor.promise.defer();
        var that = this;
        element.all(by.css('#editConfigurationsBody .se-config__entry')).then(function(rows) {
