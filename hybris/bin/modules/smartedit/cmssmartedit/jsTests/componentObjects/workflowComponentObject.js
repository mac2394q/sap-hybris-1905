...
module.exports = function() {
    ...
    componentObject.consts = {
        ...
        ITEM_MENU_WORKFLOW_CANCEL_CONFIRMATION_MESSAGE: 'When canceling workflow, the page will be moved back to Draft'
    };
    ...
    componentObject.elements = {
        ...
        getTemplateFieldOptionByName: function(name) {
            return this.getTemplateField().element(by.cssContainingText('.select2-result-single li div div', name));
        },
        ...
        getPageWorkflowMenuHeader: function() {
            return this.getPageWorkflowMenu().element(by.css('.se-page-workflow-menu__header-text'));
        },
        ...
        getItemMenuCancelItem: function() {
            return element(by.cssContainingText('div[id="cancel-workflow-item-menu"]', componentObject.consts.ITEM_MENU_WORKFLOW_CANCEL_BUTTON));
        },
        ...
        getItemMenuDescriptionHeader: function() {
            return element(by.cssContainingText('div[id="workflow-description-item-menu"]', componentObject.consts.ITEM_MENU_WORKFLOW_DESCRIPTION_HEADER));
        },

        // Workflow modal form description field
        getItemMenuEditModalFormDescriptionField: function() {
            return this.getModalForm().element(by.id('description-label', componentObject.consts.ITEM_MENU_WORKFLOW_DESCRIPTION_FIELD));
        },

        // Workflow cancel confirmation message
        getItemMenuCancelConfirmationMessage: function() {
            return this.getModalForm().element(by.id('confirmationModalDescription', componentObject.consts.ITEM_MENU_WORKFLOW_CANCEL_CONFIRMATION_MESSAGE));
        },

        // Workflow actions and comments
        getAllTasksTabLink: function() {
            return element(by.css('li[data-tab-id="allTasksTab"] a'));
        },
        getTab: function(tabId) {
            return element(by.css('y-tab[data-tab-id="' + tabId + '"]'));
        },
        getActionsForTabId: function(tabId) {
            return element.all(by.css('y-tab[data-tab-id="' + tabId + '"]' + ' workflow-action-item'));
        },
        getWorkflowActionByCode: function(tabId, actionCode) {
            return this.getTab(tabId).element(by.css('y-collapsible-container[id="' + actionCode + '"] div'));
        },
        getCommentsForAction: function(tabId, actionCode) {
            return componentObject.elements.getWorkflowActionByCode(tabId, actionCode).all(by.css('workflow-action-comment'));
        },
        getDecisionButtonsGroup: function(tabId, actionCode) {
            return componentObject.elements.getWorkflowActionByCode(tabId, actionCode).element(by.css('.se-workflow-action-item__decision-btn'));
        },
        getDecisionButtonForAction: function(actionCode, decisionName) {
            var tabId = 'currentTasksTab';
            return this.getWorkflowActionByCode(tabId, actionCode).element(by.cssContainingText('button[id="' + actionCode + '-decision-button"]', decisionName));
        },
        getSplitDecisionButtonForAction: function(actionCode) {
            var tabId = 'currentTasksTab';
            return componentObject.elements.getWorkflowActionByCode(tabId, actionCode).element(by.css('button[id="' + actionCode + '"-decision-split-button]'));
        },
    ...
    componentObject.actions = {
        ...
        expandWorkflowAction: function(tabId, actionCode) {
            return browser.click(componentObject.elements.getWorkflowActionByCode(tabId, actionCode));
        },
        clickDecisionButton: function(actionCode, decisionName, isSplitButton) {
            if (isSplitButton) {
                return browser.click(componentObject.elements.getSplitDecisionButtonForAction(actionCode, decisionName));
            }
            return browser.click(componentObject.elements.getDecisionButtonForAction(actionCode, decisionName));
        },
    ...
    componentObject.assertions = {
        ...
        modalFormIsDispalyed: function() {
            expect(componentObject.elements.getModalForm()).toBeDisplayed();
        },
        templateFieldIsDisplayed: function() {
            expect(componentObject.elements.getTemplateField()).toBeDisplayed();
            expect(componentObject.elements.getGenericEditorFieldLabel('templateCode').getText()).toContain("Workflow");
        },
        templateFieldIsAbsent: function() {
            expect(componentObject.elements.getTemplateField()).toBeAbsent();
            expect(componentObject.elements.getGenericEditorFieldLabel('templateCode')).toBeAbsent();
        },
        descriptionFieldIsDisplayed: function() {
            expect(componentObject.elements.getDescriptionField()).toBeDisplayed();
            expect(componentObject.elements.getGenericEditorFieldLabel('description').getText()).toContain("Description");
        },
        ...
        createVersionFieldIsDisplayed: function() {
            ...
            expect(componentObject.elements.getCreateVersionFieldLabel().getText()).toContain("Create page version");
        },
        ...
        versionLabelFieldContainsText: function(text) {
            browser.waitForSelectorToContainValue(componentObject.elements.getVersionLabelField(), text).then(function() {
                componentObject.elements.getVersionLabelField().getAttribute('value').then(function(actualText) {
        ...
        itemEditModalFormContainsDescriptionField: function() {
            expect(componentObject.elements.getItemMenuEditModalFormDescriptionField()).toBeDisplayed();
        },
        ...
        decisionButtonsNotDisplayed: function(tabId, actionCode) {
            expect(componentObject.elements.getDecisionButtonsGroup(tabId, actionCode)).toBeAbsent();
        },
        decisionButtonsAreDisplayed: function(tabId, actionCode) {
            expect(componentObject.elements.getDecisionButtonsGroup(tabId, actionCode)).toBeDisplayed();
        },
        ...
        assertNumberOfCommentsAvailable: function(tabId, actionCode, expectedElementCount) {
            componentObject.actions.expandWorkflowAction(tabId, actionCode).then(function() {
                componentObject.elements.getCommentsForAction(tabId, actionCode).then(function(comments) {
        ...
        commentFieldIsDisplayed: function() {
            ...
            expect(componentObject.elements.getGenericEditorFieldLabel('comment').getText()).toContain("Comment");
        }
    };

    componentObject.utils = {
        setNumberOfWorkflowsToReturn: function(count) {
            browser.executeScript('window.sessionStorage.setItem("numberOfWorkflowsToReturn", arguments[0])', count);
        },
        setWorkflowAvailableToCurrentUser: function(makeAvailable) {
            // Note: By default, the workflow is available to the current user. 
            browser.executeScript('window.sessionStorage.setItem("workflowsAvailableForCurrentPrincipal", arguments[0])', makeAvailable);
        },
        makeItemsEditable: function(items) {
            browser.executeScript('window.sessionStorage.setItem("workflowItemsEditable", arguments[0])', JSON.stringify(items));
        }
    };
