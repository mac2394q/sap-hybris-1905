...
module.exports = (function() {
    ...
    componentObject.elements = {
        ...
        visibilityListBySlotId: function(slotId) {
            return this.visibilityDropdownBySlotId(slotId).all(by.css('.se-slot-visiblity-component__content'));
        },
        ...
        hiddenComponentSharedIndicator: function(slotId, componentId) {
            return this.hiddenComponentInSlot(slotId, componentId).element(by.css('.se-slot-visibility__icon--shared'));
        },
        ...
        sharedSlotMessageButtonCloneLinksBySlotId: function() {
            return element(by.css(".se-shared-slot__link"));
        },
        sharedSlotButtonMessage: function() {
            return browser.findElement(by.css(".se-shared-slot__description")).getText().then(function(text) {
                return text;
            });
        },
        ...
        unsharedSlotButtonMessage: function() {
            browser.waitUntil(function() {
                return element.all(by.css('.se-shared-slot__link-description__unshared')).then(function(popovers) {
                    return popovers.length === 1;
                });
            }, 'no popovers are available');

            return browser.findElement(by.css('.se-shared-slot__link-description__unshared')).getAttribute("innerHTML").then(function(innerHTML) {
        ...
        syncButtonStatusBySlotId: function(slotId) {
            browser.waitForPresence(by.css("[data-smartedit-component-id='" + slotId + "'] slot-sync-button"), 'cannot find slot sync icon');
    ...
    componentObject.assertions = {
        ...
        syncButtonIsDisabled: function(slotId) {
            ...
            expect(componentObject.elements.syncButtonBySlotId(slotId).getAttribute('class')).toContain('se-slot-ctx-menu__dropdown-toggle--disabled');
        },
        ...
        assertThatSlotSharedButtonOverrideLinksToBeAbsent: function(slotId) {
            var cloneLinks = componentObject.elements.sharedSlotMessageButtonCloneLinksBySlotId(slotId);
            browser.waitForAbsence(cloneLinks);
        },

        // Hidden Components
        assertHiddenComponentIsShared: function(slotId, componentId) {
            expect(componentObject.elements.hiddenComponentSharedIndicator(slotId, componentId).isDisplayed()).toBe(true);
        },
        assertHiddenComponentIsNotShared: function(slotId, componentId) {
            expect(browser.isAbsent(componentObject.elements.hiddenComponentSharedIndicator(slotId, componentId))).toBe(true);
        }
    };
