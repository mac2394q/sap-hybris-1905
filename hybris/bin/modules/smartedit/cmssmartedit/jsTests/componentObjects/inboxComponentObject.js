...
module.exports = function() {
    ...
    componentObject.elements = {
        ...
        getInboxTaskHeader: function(taskNumber) {
            return componentObject.elements.getInboxTask(taskNumber).element(by.css('.se-cms-dev-workflow-inbox-task-header'));
        },
        getInboxTaskDescription: function(taskNumber) {
            return componentObject.elements.getInboxTask(taskNumber).element(by.css('.se-cms-dev-workflow-inbox-task-desc'));
        },
        getInboxTaskCreatedAgo: function(taskNumber) {
            return componentObject.elements.getInboxTask(taskNumber).element(by.css('.se-cms-dev-workflow-inbox-task-body'));
        },
        getInboxTaskHeaderLink: function(taskNumber) {
            return componentObject.elements.getInboxTask(taskNumber).element(by.css('.se-cms-dev-workflow-inbox-task-header a'));
        },
    ...
    componentObject.actions = {
        ...
        selectTask: function(taskNumber) {
            return browser.click(componentObject.elements.getInboxTaskHeaderLink(taskNumber)).then(function() {
    ...
    componentObject.assertions = {
        ...
        inboxTaskHasName: function(taskNumber, name) {
            expect(componentObject.elements.getInboxTaskHeader(taskNumber).getText()).toContain(name);
        },
        inboxTaskHasDescription: function(taskNumber, description) {
            expect(componentObject.elements.getInboxTaskDescription(taskNumber).getText()).toContain(description);
        },
        inboxTaskHasCreatedAgo: function(taskNumber, createdAgo) {
            expect(componentObject.elements.getInboxTaskCreatedAgo(taskNumber).getText()).toContain(createdAgo);
        },
        inboxBadgeHasUpdated: function(expectedTasksCount) {
            expect(componentObject.elements.getInboxBadgeCount().getText()).toBe(expectedTasksCount);
        }
    };

    componentObject.utils = {
        updateInboxTasks: function(canUpdateInboxTasks) {
            return browser.executeScript('window.sessionStorage.setItem("updateWorkflowInbox", arguments[0])', canUpdateInboxTasks);
        }
    };
