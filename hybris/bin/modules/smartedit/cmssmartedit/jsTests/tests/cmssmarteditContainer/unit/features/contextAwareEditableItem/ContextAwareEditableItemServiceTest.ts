...
describe('ContextAwareEditableItemServiceTest', () => {
	...
	beforeEach(() => {
		workflowService = jasmine.createSpyObj<WorkflowService>('workflowService', ['getWorkflowEditableItems', 'getActiveWorkflowForPageUuid']);
		...
		pageService.getCurrentPageInfo.and.returnValue($q.when({
			uid: PREVIEWED_PAGE_UID
		}));

		workflowService.getActiveWorkflowForPageUuid.and.returnValue($q.when({
	...
	it('should return true if the item is editable and active workflow same as the previewed page workflow', () => {
		// GIVEN
		workflowService.getWorkflowEditableItems.and.returnValue($q.when([{
			uid: ITEM_UID,
			uuid: ITEM_UID,
			editableByUser: true,
