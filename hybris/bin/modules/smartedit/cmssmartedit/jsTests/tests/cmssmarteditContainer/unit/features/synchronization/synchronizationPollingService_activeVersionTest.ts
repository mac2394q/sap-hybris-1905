...
describe('Synchronization polling service with content catalog active version - ', () => {
	...
	it('startSync call without pollingType should restart the timer with SLOW_POLLING_TIME by default', () => {
		...
		expect(timer.restart).toHaveBeenCalledWith(
			SYNCHRONIZATION_POLLING.SLOW_POLLING_TIME
		);
	});

	it('startSync call with SYNCHRONIZATION_POLLING.FAST_POLLING_TIME should restart the timer with FAST_POLLING_TIME', () => {
		timer.isActive.and.returnValue(false);

		(syncPollingService as any).startSync(SYNCHRONIZATION_POLLING.SPEED_UP);

		expect(timer.restart).toHaveBeenCalledWith(
			SYNCHRONIZATION_POLLING.FAST_POLLING_TIME
		);
	});
