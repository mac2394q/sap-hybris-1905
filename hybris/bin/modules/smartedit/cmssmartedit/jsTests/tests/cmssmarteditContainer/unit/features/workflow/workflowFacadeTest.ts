...
import {IAlertService, IPageInfoService, Page, SystemEventService, TypedMap} from 'smarteditcommons';
...
import {Workflow, WorkflowAction, WorkflowActionComment, WorkflowActionStatus, WorkflowDecision, WorkflowOperations, WorkflowStatus} from 'cmssmarteditcontainer/components/workflow/dtos';
...
describe("Test WorkflowFacade", () => {
	...
	beforeEach(() => {
		alertService = jasmine.createSpyObj<IAlertService>("alertService", ["showSuccess", "showDanger"]);
		...
		systemEventService = jasmine.createSpyObj('systemEventService', ['subscribe', 'publishAsync', 'publish']);
		workflowService = jasmine.createSpyObj('workflowService', ['getWorkflowTemplates', 'cancelWorflow', 'getResourceWorkflowURI',
			'getAllActionsForWorkflowCode', 'getActiveActionsForWorkflowCode', 'getCommentsForWorkflowAction', 'getResourceWorkflowOperationsURI']);
		...
		genericEditorModalService.open.and.callFake((componentData: any, saveCallback: (workflow: Workflow) => any) => {
			return $q.when(saveCallback(testWorkflow));
		});

		workflowService.getWorkflowTemplates.and.returnValue($q.when({
			templates: [{
	...
	describe('start workflow ', () => {
		...
		beforeEach(() => {
			...
			systemEventService.subscribe.and.callFake((eventName: string, handler: (key: string, eventData: any) => void) => {
		...
		it("WHEN start workflow is called THEN the generic editor is opened with the current page already as an attachment", () => {
			...
			// THEN
			expect(genericEditorModalService.open).toHaveBeenCalledWith(jasmine.objectContaining({
				content: {
		...
		it('WHEN workflow is started THEN a success message is shown AND the editor is properly cleaned', () => {
			...
			// THEN 
			expect(systemEventService.subscribe).toHaveBeenCalledWith(GENERIC_EDITOR_UNRELATED_VALIDATION_MESSAGES_EVENT,
				jasmine.any(Function));
		...
		it('GIVEN a workflow with a version label validation error WHEN start workflow modal is saved THEN it republishes the error to createVersion field', () => {
			...
			expect(systemEventService.publishAsync).toHaveBeenCalledWith(GENERIC_EDITOR_UNRELATED_VALIDATION_MESSAGES_EVENT, jasmine.objectContaining(expectedEventData));
		});
	});
	...
	it('WHEN getCommentsForWorkflowAction is called THEN it calls the getCommentsForworkflowAction of workflowService and copies the comments object to results', () => {
		workflowService.getCommentsForWorkflowAction.and.returnValue($q.when({
		...
		const result = workflowFacade.getCommentsForWorkflowAction(WORKFLOW_CODE, WORKFLOW_ACTION_CODE, {currentPage: 0});

		expect(workflowService.getCommentsForWorkflowAction).toHaveBeenCalledWith(WORKFLOW_CODE, WORKFLOW_ACTION_CODE, {currentPage: 0});
	...
	describe('make decision', () => {
		...
		it('GIVEN normal action WHEN makeDecision is called THEN it opens the generic editor', () => {
			// WHEN
			const result = workflowFacade.makeDecision(WORKFLOW_CODE, WORKFLOW_ACTION, WORKFLOW_DECISION);

			// THEN
			expect(genericEditorModalService.open).toHaveBeenCalledWith(jasmine.objectContaining(expectedConfig), jasmine.any(Function), null);

			expect(alertService.showSuccess).toHaveBeenCalledWith(jasmine.objectContaining({
		...
		it('GIVEN end action WHEN makeDecision is called THEN workflow completed message is displayed', () => {
			...
			// THEN 
			expect(genericEditorModalService.open).toHaveBeenCalledWith(jasmine.objectContaining(expectedConfig), jasmine.any(Function), null);
			expect(alertService.showSuccess).toHaveBeenCalledWith("se.cms.workflow.completed.alert.success");
		...
		it('GIVEN any action WHEN makeDecision is called THEN the synchronization polling speed event is triggered to force a sync job to start', () => {
			...
			// THEN 
			expect(systemEventService.publish).toHaveBeenCalledWith(SYNCHRONIZATION_POLLING.SPEED_UP, PAGE_UUID);
		});
