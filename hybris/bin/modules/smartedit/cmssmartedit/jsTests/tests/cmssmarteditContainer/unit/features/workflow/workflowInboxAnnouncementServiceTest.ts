...
describe("Test WorkflowInboxAnnouncementService", () => {
	...
	beforeEach(() => {
		workflowTasksPollingService = jasmine.createSpyObj('workflowTasksPollingService', ['addSubscriber']);
	...
	it('Should call addSubscriber when initialized', () => {
		expect(workflowTasksPollingService.addSubscriber).toHaveBeenCalledWith(jasmine.any(Function), false);
	});
	...
	it('Should show single task announcement', () => {
		...
		// THEN
		expect((workflowInboxAnnouncementService as any)._showSingleTaskAnnouncement).toHaveBeenCalledWith(tasks[0]);
	});
	...
	it('Should show multiple tasks announcement', () => {
		...
		expect((workflowInboxAnnouncementService as any)._showMultipleTasksAnnouncement).toHaveBeenCalledWith(tasks);
	});
