...
describe('seMediaAdvancedProperties', function() {
    ...
    describe('template', function() {
        it('should have a link with translated text', function() {
            expect(window.smarteditJQuery(element.find('.se-media__advanced-info .se-media-format__label')).text()).toContain('Information');
        });

        it('should trigger bootstrap popover', function() {
            expect(window.smarteditJQuery(element.find('.se-media__advanced-info')).attr('data-uib-popover-template')).toBeTruthy();
        });
    });
});
