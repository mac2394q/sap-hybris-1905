...
describe('editorModalService', () => {
	...
	beforeEach(() => {
		...
		genericEditorModalServiceMock = jasmine.createSpyObj('genericEditorModalServiceMock', ['open']);
		contextAwareEditableItemService = jasmine.createSpyObj('contextAwareEditableItemService', ['isItemEditable']);
		...
		genericEditorModalServiceMock.open.and.callFake(
			(componentData: IGenericEditorModalServiceComponent) => {
				const resolvedValue = ((componentData && componentData.content) ? componentData.content.slotId : undefined) as string;
				return promiseHelper.buildPromise<string>('genericEditorModalServiceMock', PromiseType.RESOLVES, resolvedValue);
			}
	...
	it('open will delegate to genericEditorModalService.open and invoke a rerendering upon closing', () => {
		...
		const genericEditorModalServiceEditComponentMock: IGenericEditorModalServiceComponent = {
			...
			title: 'type.' + editorModalServiceEditComponentMock.smarteditComponentType.toLowerCase() + '.name',
	...
	it('GIVEN creating a new component THEN renderService.renderComponent is not called', () => {
		...
		const genericEditorModalServiceCreateComponentMock: IGenericEditorModalServiceComponent = {
			...
			title: 'type.' + editorModalServiceCreateComponentMock.smarteditComponentType.toLowerCase() + '.name',
			targetedQualifier: undefined,
			content: {
				slotId: undefined,
		...
		// THEN
		expect(renderServiceMock.renderSlots).not.toHaveBeenCalled();
		expect(genericEditorModalServiceMock.open.calls.argsFor(0)[0]).toEqual(genericEditorModalServiceCreateComponentMock);
		const callback = genericEditorModalServiceMock.open.calls.argsFor(0)[1];
		callback();
		expect(renderServiceMock.renderSlots).not.toHaveBeenCalled();
	});
	...
	it("should display message if component is editable in current context", () => {
		...
		// THEN
		expect($translate.instant).toHaveBeenCalledWith(COMPONENT_SHARED_IN_WORKFLOW_CONTEXT_MESSAGE);
		const genericEditorParams = genericEditorModalServiceMock.open.calls.argsFor(0)[0];
		expect(genericEditorParams.messages).toContain({
			type: "info",
