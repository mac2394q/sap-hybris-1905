...
describe('Page Synchronization Header Component Test', () => {
	...
	let l10nFilter: (s: string) => string;
	...
	beforeEach(() => {
		...
		catalogService = jasmine.createSpyObj<ICatalogService>('catalogService', ['getActiveContentCatalogVersionByCatalogId']);
		l10nFilter = (s: string) => s;
	...
	it('should display the description when the sync page ', function() {
		...
		catalogService.getActiveContentCatalogVersionByCatalogId.and.returnValue($q.resolve('CATALOG_VERSION_UUID'));
		cmsitemsRestService.getByIds.and.returnValue($q.resolve({
			response: [
				{name: 'ITEM_ID_1_PAGE'},
				{name: 'ITEM_ID_2_PAGE'}
			]
		...
		expect(catalogService.getActiveContentCatalogVersionByCatalogId).toHaveBeenCalledWith('CATALOG_ID');
		expect(cmsitemsRestService.getByIds).toHaveBeenCalledWith(['ITEM_ID_1', 'ITEM_ID_2']);
		expect($translate.instant.calls.argsFor(0)[0]).toBe('se.cms.synchronization.page.unavailable.items.description');
	...
	it('should display the description when the page has not been synchronization yet ', function() {
		...
		expect(sharedDataService.get).toHaveBeenCalledWith('experience');
		expect(catalogService.getActiveContentCatalogVersionByCatalogId).toHaveBeenCalledWith('DESCRIPTOR_CATALOG_ID');
		expect(cmsitemsRestService.getByIds).not.toHaveBeenCalled();
		expect($translate.instant.calls.argsFor(0)[0]).toBe('se.cms.synchronization.page.new.description');
		expect($translate.instant.calls.argsFor(0)[1]).toEqual({
