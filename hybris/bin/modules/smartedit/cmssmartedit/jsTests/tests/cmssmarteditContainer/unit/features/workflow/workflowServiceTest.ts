...
import {annotationService, pageChangeEvictionTag, perspectiveChangedEvictionTag, rarelyChangingContent, Cached, CrossFrameEventService, ICatalogService, IExperience, IExperienceService, IPerspectiveService, IRestService, IRestServiceFactory, ISharedDataService, Page, Pagination, SystemEventService, WindowUtils} from 'smarteditcommons';
import {promiseHelper} from 'testhelpers';
import {Workflow, WorkflowAction, WorkflowActionComment, WorkflowStatus, WorkflowTask, WorkflowTemplate} from 'cmssmarteditcontainer/components/workflow/dtos';
import {OpenPageWorkflowMenu, WorkflowService} from 'cmssmarteditcontainer/components/workflow/services/WorkflowService';
import {WorkflowTasksPollingService} from 'cmssmarteditcontainer/components/workflow/services/WorkflowTasksPollingService';
import {workflowCompletedEvictionTag, workflowTasksMenuOpenedEvictionTag, CMSModesService} from 'cmscommons';
...
describe("Test WorkflowService", () => {
	...
	let workflowActionCommentsRESTService: jasmine.SpyObj<IRestService<Page<WorkflowActionComment>>>;
	...
	beforeEach(() => {
		...
		experienceService = jasmine.createSpyObj<IExperienceService>("experienceService", ["getCurrentExperience", "compareWithCurrentExperience", "loadExperience"]);
		crossFrameEventService = jasmine.createSpyObj<CrossFrameEventService>("crossFrameEventService", ["subscribe", "publish"]);
		sharedDataService = jasmine.createSpyObj<ISharedDataService>("sharedDataService", ["set", "get", "remove"]);
		perspectiveService = jasmine.createSpyObj<IPerspectiveService>("perspectiveService", ["getActivePerspectiveKey", "switchTo"]);
		systemEventService = jasmine.createSpyObj<SystemEventService>("systemEventService", ["subscribe", "publish"]);
		catalogService = jasmine.createSpyObj<ICatalogService>('catalogService', ["retrieveUriContext", "getDefaultSiteForContentCatalog"]);
		windowUtils = jasmine.createSpyObj<WindowUtils>('windowUtils', ["getTargetIFrame"]);
		workflowTasksPollingService = jasmine.createSpyObj<WorkflowTasksPollingService>('workflowTasksPollingService', ["addSubscriber"]);
	...
	describe("Open Page Workflow Menu", () => {
		...
		it("WHEN initialized THEN subscribes to EVENT_PERSPECTIVE_REFRESHED and EVENT_PERSPECTIVE_CHANGED event", () => {
			...
			expect(workflowTasksPollingService.addSubscriber).toHaveBeenCalledWith(jasmine.any(Function), true);
		});
		...
		it("WHEN currently in basic edit perspective mode THEN publishes an event to open the menu", () => {
			...
			perspectiveService.getActivePerspectiveKey.and.returnValue($q.resolve(CMSModesService.BASIC_PERSPECTIVE_KEY));

			// WHEN
			openPageWorkflowMenu();

			// THEN
			expect(systemEventService.publish).toHaveBeenCalledWith(CMS_EVENT_OPEN_PAGE_WORKFLOW_MENU, true);
		...
		it("WHEN currently in versioning perspective mode THEN switches to advanced mode and sets OPEN_PAGE_WORKFLOW_MENU in sharedDataService to SwitchPerspective", () => {
			...
			const openPageWorkflowMenu = crossFrameEventService.subscribe.calls.argsFor(0)[1];
			sharedDataService.get.and.returnValue($q.resolve(OpenPageWorkflowMenu.Default));
			perspectiveService.getActivePerspectiveKey.and.returnValue($q.resolve(CMSModesService.VERSIONING_PERSPECTIVE_KEY));
			...
			// THEN
			expect(sharedDataService.set).toHaveBeenCalledWith(OPEN_PAGE_WORKFLOW_MENU, OpenPageWorkflowMenu.SwitchPerspective);
			expect(perspectiveService.switchTo).toHaveBeenCalledWith(CMSModesService.ADVANCED_PERSPECTIVE_KEY);
		});

		it("WHEN OPEN_PAGE_WORKFLOW_MENU is SwitchPerspective THEN it publishes an event to open the menu", () => {
			// GIVEN
			const openPageWorkflowMenu = crossFrameEventService.subscribe.calls.argsFor(0)[1];
			sharedDataService.get.and.returnValue($q.resolve(OpenPageWorkflowMenu.SwitchPerspective));
		...
		it("WHEN given experience from workflow task is same as current experience THEN it does not load experience", () => {
			...
			catalogService.getDefaultSiteForContentCatalog.and.returnValue($q.when({uid: 'siteId'}));
			experienceService.compareWithCurrentExperience.and.returnValue($q.when(true));
			sharedDataService.set.and.returnValue($q.when());
			windowUtils.getTargetIFrame.and.returnValue(true);
			...
			expect(sharedDataService.set).toHaveBeenCalledWith(OPEN_PAGE_WORKFLOW_MENU, OpenPageWorkflowMenu.Default);
		...
		it("WHEN there is no current experience THEN it loads the experience from the given parameters", () => {
			...
			experienceService.compareWithCurrentExperience.and.returnValue($q.when(false));
			experienceService.loadExperience.and.returnValue($q.when());
			windowUtils.getTargetIFrame.and.returnValue(null);
	...
	describe("Workflow cancel", () => {
		...
		it("WHEN cancel workflow is called THEN update rest call is made", () => {
			// GIVEN
			workflowOperationsRESTService = jasmine.createSpyObj<IRestService<Workflow>>("workflowOperationsRESTService", ["save"]);
	...
	describe('Workflow templates', () => {
		beforeEach(() => {
			workflowTemplateRESTService = jasmine.createSpyObj<IRestService<Workflow>>("workflowTemplateRESTService", ["get"]);
			restServiceFactory.get.and.returnValue(workflowTemplateRESTService);
			workflowTemplateRESTService.get.and.returnValue($q.when({
				templates: []
			}));
	...
	describe("Active workflow for page", () => {
		...
		beforeEach(() => {
			workflowRESTService = jasmine.createSpyObj<IRestService<Workflow>>("workflowRESTService", ["get"]);
	...
	describe("Workflow search", () => {
		...
		it('WHEN getWorkflows is called THEN it should be cached', () => {
			// WHEN / THEN
			const decoratorObj: any = annotationService.getMethodAnnotation(WorkflowService, 'getWorkflows', Cached);
			expect(decoratorObj).toEqual(jasmine.objectContaining([{
				actions: [rarelyChangingContent],
				tags: [pageChangeEvictionTag, perspectiveChangedEvictionTag, workflowTasksMenuOpenedEvictionTag, workflowCompletedEvictionTag]
			}]));
		});
	...
	describe("Workflow Template search", () => {
		...
		it('WHEN getWorkflowTemplates is called THEN it should be cached', () => {
			// WHEN / THEN
			const decoratorObj: any = annotationService.getMethodAnnotation(WorkflowService, 'getWorkflowTemplates', Cached);
			expect(decoratorObj).toEqual(jasmine.objectContaining([{
				actions: [rarelyChangingContent],
	...
	describe("Actions For Workflow Code", () => {
		...
		beforeEach(() => {
			...
			workflowActionsRESTService = jasmine.createSpyObj<IRestService<Workflow>>("workflowActionsRESTService", ["get"]);
			restServiceFactory.get.and.returnValue(workflowActionsRESTService);
			workflowActionsRESTService.get.and.returnValue($q.when({
		...
		it('WHEN getAllActionsForWorkflowCode is called THEN it should be cached', () => {
			// WHEN / THEN
			const decoratorObj: any = annotationService.getMethodAnnotation(WorkflowService, 'getAllActionsForWorkflowCode', Cached);
			expect(decoratorObj).toEqual(jasmine.objectContaining([{
				actions: [rarelyChangingContent],
				tags: [pageChangeEvictionTag, workflowTasksMenuOpenedEvictionTag, workflowCompletedEvictionTag]
			}]));
		});
	...
	describe("Comments For Workflow Action", () => {
		...
		beforeEach(() => {
			workflowActionCommentsRESTService = jasmine.createSpyObj<IRestService<Workflow>>("workflowActionCommentsRESTService", ["get"]);
			restServiceFactory.get.and.returnValue(workflowActionCommentsRESTService);
			workflowActionCommentsRESTService.get.and.returnValue($q.when({
		...
		it('WHEN getCommentsForworkflowAction is called THEN should return all available comments associated to a given action', () => {
			// WHEN
			const result = workflowService.getCommentsForWorkflowAction("someWorkflowCode", "someWorkflowAction", {
				currentPage: 0
			});
