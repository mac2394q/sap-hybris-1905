...
import {annotationService, pageDeletionEvictionTag, pageRestoredEvictionTag, rarelyChangingContent, Cached, CrossFrameEventService, IAlertService, ICatalogService, IPageInfoService, IRestService, IRestServiceFactory} from 'smarteditcommons';
...
describe('managePageService', () => {
	...
	const workflowService: jasmine.SpyObj<WorkflowService> = jasmine.createSpyObj<WorkflowService>('workflowService', ['isPageInWorkflow']);
	...
	const catalogService = jasmine.createSpyObj<ICatalogService>('catalogService', ['retrieveUriContext', 'getContentCatalogActiveVersion']);
	const pageOperationsRESTService = jasmine.createSpyObj<IRestService<any>>('pageOperationsRESTService', ["save"]);
	...
	describe('trashPageInActiveCatalogVersion', () => {
		...
		it('GIVEN a page WHEN trashPageInActiveCatalogVersion  is calleds THEN it will tash the page in the active catalog version', () => {
			...
			catalogService.getContentCatalogActiveVersion.and.returnValue($q.when(ACTIVE_CATALOG_VERSION));
			confirmationModalService.confirm.and.returnValue($q.resolve());
			...
			expect(catalogService.getContentCatalogActiveVersion).toHaveBeenCalledWith(MOCK_URI_CONTEXT);
