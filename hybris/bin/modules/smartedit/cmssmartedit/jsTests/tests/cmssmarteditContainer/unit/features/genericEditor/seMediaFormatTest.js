    ...
    describe('template', function() {
        ...
        describe('when media uuid present', function() {
            it('should show the media present view', function() {
                expect(element.find('.se-media--present')).toExist();
                expect(element.find('.se-media--absent')).not.toExist();
                expect(element.find('.se-media--edit')).not.toExist();
            });

            it('should show the image', function() {
                expect(element.find('.se-media-preview__image-thumbnail').attr('data-ng-src')).toBe('/web/webroot/images/edit_icon.png');
            });

            it('should show a replace button', function() {
                expect(element.find('.se-media__remove-btn-txt').text().trim()).toContain('se.media.format.remove');
            });
        });
        ...
        describe('when media uuid absent', function() {
            ...
            it('should show the media absent view', function() {
                expect(element.find('.se-media--present')).not.toExist();
                expect(element.find('.se-media--absent')).toExist();
                expect(element.find('.se-media--edit')).not.toExist();
            });

            it('should show an upload button', function() {
                expect(element.find('.se-media--absent se-file-selector .label__fileUpload-link').text().trim()).toBe('Upload');
            });
            ...
            describe('when field.editable is false', function() {
                ...
                it('should se-file-selector have file-selector-disabled class', function() {
                    expect(element.find('.se-media--absent se-file-selector').isolateScope().ctrl.customClass).toContain('file-selector-disabled');
                });
            });
        });
        ...
        describe('when under edit', function() {
            ...
            it('should show the media uploading view', function() {
                expect(element.find('.se-media--present')).not.toExist();
                expect(element.find('.se-media--absent')).not.toExist();
                expect(element.find('.se-media--edit')).toExist();
            });

            it('should show the editing text', function() {
                expect(element.find('.se-media--edit').text().trim()).toContain('Upload');
            });
        });
        ...
        describe('when media uuid is present and field is disabled', function() {
            ...
            it('should se-file-selector have file-selector-disabled class', function() {
                expect(element.find('.se-media--present se-file-selector').isolateScope().ctrl.customClass).toContain('file-selector-disabled');
            });
            ...
            it('remove button should be disabled', function() {
                ...
                // WHEN/THEN
                expect(element.find('.se-media--present .se-media__remove-btn').prop('disabled')).toBe(true);
            });
        });
