...
describe('AttributePermissionsRestService', () => {
	...
	const restService: jasmine.SpyObj<IRestService<any>> = jasmine.createSpyObj<IRestService<any>>('typePermissionsRestResource', ['get']);
	...
	beforeEach(() => {
		...
		attributePermissionsRestService = new AttributePermissionsRestService(restServiceFactory, sessionService, $q, $log, lo);
	});
	...
	it('GIVEN attributes are found WHEN hasReadPermissionOnAttributesInType is called THEN the promise is resolved with a typed map with the right permissions', () => {
		// WHEN 
		const promise = attributePermissionsRestService.hasReadPermissionOnAttributesInType(TYPE_1, [ATTR_1, ATTR_2]) as IExtensiblePromise<TypedMap<boolean>>;
	...
	it('GIVEN attribute is not found WHEN hasReadPermissionOnAttributesInType is called THEN the promise is rejected', () => {
		...
		// WHEN 
		const promise = attributePermissionsRestService.hasReadPermissionOnAttributesInType(TYPE_1, [ATTR_1, ATTR_2]) as IExtensiblePromise<string>;
	...
	it('GIVEN attributes are found WHEN hasChangePermissionOnAttributesInType is called THEN the promise is rejected with the right permissions', () => {
		// WHEN 
		const promise = attributePermissionsRestService.hasChangePermissionOnAttributesInType(TYPE_1, [ATTR_1, ATTR_2]) as IExtensiblePromise<TypedMap<boolean>>;
	...
	it('GIVEN attribute is not found WHEN hasChangePermissionOnAttributesInType is called THEN the promise is rejected', () => {
		...
		// WHEN 
		const promise = attributePermissionsRestService.hasChangePermissionOnAttributesInType(TYPE_1, [ATTR_1, ATTR_2]) as IExtensiblePromise<string>;
