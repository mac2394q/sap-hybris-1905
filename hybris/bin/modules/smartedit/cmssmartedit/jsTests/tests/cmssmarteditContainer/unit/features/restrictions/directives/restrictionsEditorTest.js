...
describe('restrictionsEditor', function() {
    ...
    var template = '<restrictions-editor data-editable="editable" data-item="item" data-restrictions="restrictions" data-on-criteria-selected="onCriteriaSelected"></restrictions-editor>';
    ...
    beforeEach(function() {
        var harness = AngularUnitTestHelper.prepareModule('restrictionsEditorModule')
            ...
            .mock('contextAwareEditableItemService', 'isItemEditable').and.returnValue(new Promise(function(resolve, reject) {
                resolve(false);
            }))
            .mock('isBlank')
            .component(template, {
    ...
    describe('restrictions list', function() {
        ...
        it('should render the name, type, and description of each restriction', function() {
            expect(element.find('#restriction-1 .se-restriction__item-name').text()).toBe('Sample Name');
            expect(element.find('#restriction-1 .se-restriction__item-type-and-id').text()).toContain('SampleRestriction');
            expect(element.find('#restriction-1 .se-restriction__item-description').text()).toBe('Sample Description');
        });
    });
    ...
    describe('read-only criteria', function() {
        ...
        it('should render [Match all] criteria by default', function() {
            ...
            expect(element.find('.se-restriction-criteria').text()).toContain('Criteria: Match all');
        });
    });
