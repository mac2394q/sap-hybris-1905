...
describe('componentInfoService', function() {
    ...
    it('WHEN components are added THEN getById should return the component data', function() {
        ...
        expect(componentInfoService.getById(COMPONENT1.uuid)).toBeResolvedWithData(getExpectedComponentData(COMPONENT1.uuid));
    });
    ...
    it('WHEN getById is called and the component data is not cached, it should resolve when the component data is ready', function() {
        ...
        $rootScope.$digest();
        expect(componentInfoService.getById(COMPONENT2.uuid)).toBeResolvedWithData(getExpectedComponentData(COMPONENT2.uuid));
    ...
    it('WHEN getById is called with the forceImmediateResult enabled THEN it should query the backend right away AND return the component data', function() {
        ...
        // THEN 
        expect(componentInfoService.getById(COMPONENT1.uuid, true)).toBeResolvedWithData(COMPONENT1);
    ...
    it('should subscribe to COMPONENT_CREATED_EVENT', function() {
        expect(crossFrameEventService.subscribe).toHaveBeenCalledWith("COMPONENT_CREATED_EVENT", jasmine.any(Function));
    });

    it('should subscribe to COMPONENT_UPDATED_EVENT', function() {
        expect(crossFrameEventService.subscribe).toHaveBeenCalledWith("COMPONENT_UPDATED_EVENT", jasmine.any(Function));
    });

    it('should subscribe to COMPONENT_REMOVED_EVENT', function() {
        expect(crossFrameEventService.subscribe).toHaveBeenCalledWith("COMPONENT_REMOVED_EVENT", jasmine.any(Function));
    });
