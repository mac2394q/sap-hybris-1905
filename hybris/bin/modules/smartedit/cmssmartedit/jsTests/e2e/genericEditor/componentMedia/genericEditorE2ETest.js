...
describe("GenericEditor Media Upload", function() {
    ...
    it('GIVEN a Media structure type is present in the generic editor ' +
        ...
        function() {
            selectFileToUpload('more_bckg.png').then(function() {
                browser.wait(EC.presenceOf(element(by.css(getCssForEnglish('.se-media-upload__file-name')))), 5000, 'Timed out waiting for presence of file name element');
                expect(element(by.css(getCssForEnglish('.se-media-upload__file-name'))).getText()).toBe('more_bckg.png');
    ...
    it('GIVEN a Media structure type is present in the generic editor ' +
        ...
        function() {
            selectFileToUpload('more_bckg.png').then(function() {
                ...
                expect(element(by.css(getCssForEnglish('.se-media-preview__image-thumbnail'))).getAttribute('src')).toContain('web/webroot/static-resources/images/more_bckg.png');
                expect(element(by.css(getCssForEnglish('.se-media-preview__image-thumbnail'))).getAttribute('data-ng-src')).toContain('web/webroot/static-resources/images/more_bckg.png');
            });
        });
    ...
    it('GIVEN a media is selected WHEN I click the preview button THEN I expect to see a popover with the image in it.', function() {
        browser.click(by.css(getCssForEnglish(".se-media-preview__icon"))).then(function() {
            browser.wait(EC.visibilityOf(element(by.css(".se-media-preview__image"))), 5000);
            expect(element(by.css(".se-media-preview__image")).isDisplayed()).toBe(true, 'Exepcted preview image container to be displayed');
            expect(element(by.css(".se-media-preview__image")).getAttribute("src")).toContain("contextualmenu_delete_off.png", 'Expected preview image to be displayed');
            expect(element(by.css(".se-media-preview__image")).getAttribute("data-ng-src")).toContain("contextualmenu_delete_off.png", 'Expected preview image to be displayed');
        });
    });
    ...
    function selectFileToUpload(fileName) {
        return browser.click(by.css(getCssForEnglish(".se-media-printer__btn.remove-image")), 5000, 'Remove image button not present').then(function() {
