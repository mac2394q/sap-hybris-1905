 ...
 */
angular
    .module('workflowMocks', ['ngMockE2E'])
    .run(
        function($httpBackend, parseQuery, backendMocksUtils) {
            ...
            var workflowTemplatesGETMock = $httpBackend.whenGET(/\/cmswebservices\/v1\/catalogs\/.*\/versions\/.*\/workflowtemplates/).respond(function() {
                return [200, {
            ...
            var workflowSearchGETMock = $httpBackend.whenGET(/\/cmswebservices\/v1\/catalogs\/.*\/versions\/.*\/workflows\?attachment=.*?&currentPage=0&pageSize=1&statuses=running,paused/).respond(function() {
                var numberOfWorkflowsToReturn = JSON.parse(sessionStorage.getItem('numberOfWorkflowsToReturn'));
                var workflowsAvailableForCurrentPrincipal = (sessionStorage.getItem('workflowsAvailableForCurrentPrincipal')) ?
                    JSON.parse(sessionStorage.getItem('workflowsAvailableForCurrentPrincipal')) : true;

                numberOfWorkflowsToReturn = numberOfWorkflowsToReturn === null ? 1 : numberOfWorkflowsToReturn;
            ...
            var workflowCommentsGETMock = $httpBackend.whenGET(/\/cmswebservices\/v1\/catalogs\/.*\/versions\/.*\/workflows\/.*\/actions\/.*\/comments/).respond(function(method, url) {
                ...
                if (workflowInfo.workflowActionCode === 'Action1') {
                    comments.push(createRegularComment('Comment1', 'This is the 1st workflow comment'),
                        createDecisionComment('Action1', 'DecisionComment1'));
                }
                if (workflowInfo.workflowActionCode === 'Action2') {
                    comments.push(createRegularComment('Comment2', 'please fix the component names'),
                        createDecisionComment('Action1', 'DecisionComment1'),
                ...
                if (workflowInfo.workflowActionCode === 'Action3') {
                    comments.push(createRegularComment('Comment9', 'Good work'),
            ...
            // WORKFLOW ACTIONS
            var workflowActionsGETMock = $httpBackend.whenGET(/\/cmswebservices\/v1\/catalogs\/.*\/versions\/.*\/workflows\/.*\/actions/).respond(function(method, url) {
            ...
            // WORKFLOWS
            var workflowGETMock = $httpBackend.whenGET(/\/cmswebservices\/v1\/catalogs\/.*\/versions\/.*\/workflows\/.*/).respond(function(method, url) {
            ...
            // WORKFLOW OPERATIONS
            var workflowOperationsPOSTMock = $httpBackend.whenPOST(/\/cmswebservices\/v1\/catalogs\/.*\/versions\/.*\/workflows\/.*\/operations/).respond(function(method, url, data) {
                ...
                workflowActions.splice(index, 1, createAction('Action2', WorkflowActionType.NORMAL, WorkflowActionStatus.COMPLETED, true));

                var index2 = workflowActions.findIndex(function(action) {
                    return action.code === 'Action4';
                });
                workflowActions.splice(index2, 1, createAction('Action4', WorkflowActionType.END, WorkflowActionStatus.IN_PROGRESS, true));
            ...
            backendMocksUtils.storeBackendMock('workflowOperationsPOSTMock', workflowOperationsPOSTMock);

            var workflowEditableItemssGETMock = $httpBackend.whenGET(/\/cmssmarteditwebservices\/v1\/catalogs\/.*\/versions\/.*?\/workfloweditableitems\?itemUids=.*/).respond(function(method, url) {
                ...
                var workflowItemsEditable = sessionStorage.getItem('workflowItemsEditable') ?
                    JSON.parse(sessionStorage.getItem('workflowItemsEditable')) : itemUids;

                var data = itemUids.map(function(uid) {
                    return {
                        uid: uid,
                        uuid: uid,
                        editableByUser: workflowItemsEditable.findIndex(function(editableUid) {
            ...
            backendMocksUtils.storeBackendMock('workflowEditableItemssGETMock', workflowEditableItemssGETMock);
            ...
            var workflowActions = [createAction('Action1', WorkflowActionType.NORMAL, WorkflowActionStatus.COMPLETED, true),
