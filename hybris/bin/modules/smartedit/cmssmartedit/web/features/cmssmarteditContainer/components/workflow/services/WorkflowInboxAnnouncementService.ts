...
export class WorkflowInboxAnnouncementService {
	...
	constructor(
		...
		this.unsubscribePollingService = this.workflowTasksPollingService.addSubscriber((tasks: WorkflowTask[], pagination: Pagination) => {
			if (tasks) {
	...
	private _showSingleTaskAnnouncement(task: WorkflowTask) {
		...
		function showAnnouncementController(cMSTimeService: CMSTimeService, announcementService: IAnnouncementService, workflowService: WorkflowService) {
			'ngInject';
			this.task = task;
			this.getStartedAgo = () => {
		...
		this._showAnnouncement('workflowInboxSingleTaskAnnouncementTemplate.html', showAnnouncementController).then((id: string) => {
			announcementId = id;
		});
	}
	...
	private _showMultipleTasksAnnouncement(tasks: WorkflowTask[]) {
		...
		function showAnnouncementController(systemEventService: SystemEventService, announcementService: IAnnouncementService, CMS_EVENT_OPEN_WORKFLOW_INBOX_DROPDOWN: string) {
			'ngInject';
		...
		this._showAnnouncement('workflowInboxMultipleTasksAnnouncementTemplate.html', showAnnouncementController).then((id: string) => {
			announcementId = id;
		});
	}

	private _showAnnouncement(templateUrl: string, controller: angular.IControllerConstructor): angular.IPromise<string> {
