...
import {ICatalogService, IExperience, IExperiencePageContext, ISeComponent, ISharedDataService, SeComponent, TypedMap} from 'smarteditcommons';
...
export class PageSynchronizationHeaderComponent implements ISeComponent {
	...
	constructor(
		...
		private l10nFilter: (s: string) => string,
	...
	$onChanges(): void {
		...
		let catalog: TypedMap<string> = null;
		let catalogId: string = null;
		let catalogName: string = null;
		let catalogVersion: string = null;

		this.sharedDataService.get('experience').then((data: IExperience) => {
			...
			this.headerText = this.pageSyncConditions.pageHasUnavailableDependencies ?
				this.$translate.instant('se.cms.synchronization.page.unavailable.items.description', {
					itemNames,
					catalogName,
					catalogVersion
	...
	private getCurrentCatalogIdFromExperience({pageContext, catalogDescriptor}: IExperience): TypedMap<string> {
		return {
			catalogId: this.userIsInsidePage(pageContext) ? pageContext.catalogId : catalogDescriptor.catalogId,
			catalogName: this.userIsInsidePage(pageContext) ? pageContext.catalogName : catalogDescriptor.name
		};
	}

	private fetchUnavailableDependencies(): string {
		return this.cmsitemsRestService.getByIds(this.syncStatus.unavailableDependencies.map(({itemId}: ISyncStatus) => itemId))
			.then(({response}: {response: ICMSPage[]}) => response.map(({name}) => name).join(', '));
	}
}