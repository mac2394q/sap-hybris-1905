...
import {CrossFrameEventService, ISeComponent, IWaitDialogService, SeComponent, TypedMap} from 'smarteditcommons';
...
export class PageApprovalSelectorComponent implements ISeComponent {
	...
	constructor(
		...
		this.unregWfFinishedHandler = this.crossFrameEventService.subscribe(this.WORKFLOW_FINISHED_EVENT, this.hideComponentIfWorkflowInProgress.bind(this));
		this.unregPerspectiveChangedHandler = this.crossFrameEventService.subscribe(this.EVENT_PERSPECTIVE_CHANGED, this.hideComponentIfWorkflowInProgress.bind(this));

		iframeClickDetectionService.registerCallback('pageApprovalSelectorClose', this.closeDropdown.bind(this));
	}
	...
	public selectApprovalStatus(approvalStatus: PageApprovalOption): void {
		...
		this.pageService.forcePageApprovalStatus(approvalStatus.key).then(() => {
	...
	private hideComponentIfWorkflowInProgress(): void {
		this.pageService.getCurrentPageInfo().then((pageInfo: ICMSPage) => {
			return this.workflowService.getActiveWorkflowForPageUuid(pageInfo.uuid).then((workflow: Workflow) => {
	...
	private getCurrentPageApprovalStatus(): angular.IPromise<void> {
		return this.pageService.getCurrentPageInfo().then((currentPageInfo: ICMSPage) => {
			...
			// Reset the options. 
			this.lodash.values(this.pageApprovalOptions).forEach((option: PageApprovalOption) => option.selected = false);
