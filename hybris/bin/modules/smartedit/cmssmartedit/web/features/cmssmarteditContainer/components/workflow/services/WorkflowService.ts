...
import {workflowCompletedEvictionTag, workflowTasksMenuOpenedEvictionTag, CMSModesService, ICMSPage} from 'cmscommons';
...
export class WorkflowService {
	...
	private workflowActionCommentsRESTService: IRestService<Page<WorkflowActionComment>>;
	private workflowEditableItemsRESTService: IRestService<WorkflowEditableItemsList>;
	private workflowInboxTasksRESTService: IRestService<Page<WorkflowTask>>;
	...
	constructor(
		...
		this.resourceWorkflowURI = `/cmswebservices/v1/catalogs/${this.PAGE_CONTEXT_CATALOG}/versions/${this.PAGE_CONTEXT_CATALOG_VERSION}/workflows`;
		this.resourceWorkflowActionsURI = `/cmswebservices/v1/catalogs/${this.PAGE_CONTEXT_CATALOG}/versions/${this.PAGE_CONTEXT_CATALOG_VERSION}/workflows/:workflowCode/actions`;
		this.resourceWorkflowTemplateURI = `/cmswebservices/v1/catalogs/${this.PAGE_CONTEXT_CATALOG}/versions/${this.PAGE_CONTEXT_CATALOG_VERSION}/workflowtemplates`;
		this.resourceWorkflowOperationsURI = `/cmswebservices/v1/catalogs/${this.PAGE_CONTEXT_CATALOG}/versions/${this.PAGE_CONTEXT_CATALOG_VERSION}/workflows/:workflowCode/operations`;
		this.resourceWorkflowActionCommentsURI = `/cmswebservices/v1/catalogs/${this.PAGE_CONTEXT_CATALOG}/versions/${this.PAGE_CONTEXT_CATALOG_VERSION}/workflows/:workflowCode/actions/:actionCode/comments`;
		this.resourceWorkflowEditableItemsURI = `/cmssmarteditwebservices/v1/catalogs/${this.PAGE_CONTEXT_CATALOG}/versions/${this.PAGE_CONTEXT_CATALOG_VERSION}/workfloweditableitems`;
		...
		this.crossFrameEventService.subscribe(this.EVENT_PERSPECTIVE_REFRESHED, this.openPageWorkflowMenu.bind(this));
		this.crossFrameEventService.subscribe(this.EVENT_PERSPECTIVE_CHANGED, this.openPageWorkflowMenu.bind(this));

		this.workflowTasksPollingService.addSubscriber((tasks: WorkflowTask[], pagination: Pagination) => {
			this.totalNumberOfTasks = pagination.totalCount || 0;
			this.crossFrameEventService.publish(this.WORKFLOW_TASKS_COUNT_UPDATED, this.totalNumberOfTasks);
		}, true);
	}
	 ...
	 * request fails, it resolves with errors from the backend.
	 */
	@Cached({actions: [rarelyChangingContent], tags: [pageChangeEvictionTag, perspectiveChangedEvictionTag, workflowTasksMenuOpenedEvictionTag, workflowCompletedEvictionTag]})
	public getWorkflows(queryParams: Payload): angular.IPromise<Workflow[]> {
		return this.workflowRESTService.get(queryParams).then((response: WorkflowList) => {
	...
	public getWorkflowTemplates(queryParams: Payload): angular.IPromise<WorkflowTemplate[]> {
		return this.workflowTemplateRESTService.get(queryParams).then((response: WorkflowTemplateList) => {
	 ...
	 * request fails, it resolves with errors from the backend.
	 */
	@Cached({actions: [rarelyChangingContent], tags: [pageChangeEvictionTag, workflowTasksMenuOpenedEvictionTag, workflowCompletedEvictionTag]})
	public getAllActionsForWorkflowCode(workflowCode: string): angular.IPromise<WorkflowAction[]> {
		return this.workflowActionsRESTService.get({
			workflowCode
	 ...
	 * request fails, it resolves with errors from the backend.
	 */
	public getActiveActionsForWorkflowCode(workflowCode: string): angular.IPromise<WorkflowAction[]> {
		return this.getAllActionsForWorkflowCode(workflowCode).then((actions: WorkflowAction[]) => {
			return actions.filter((action) => action.isCurrentUserParticipant &&
				(WorkflowActionStatus.IN_PROGRESS === action.status.toLowerCase() || WorkflowActionStatus.PAUSED === action.status.toLowerCase())
			);
		});
	}
	 ...
	 * request fails, it resolves with errors from the backend.
	 */
	public getCommentsForWorkflowAction(workflowCode: string, workflowActionCode: string, payload: Pageable): angular.IPromise<Page<WorkflowActionComment>> {
		this.workflowActionCommentsRESTService = this.restServiceFactory.get(this.resourceWorkflowActionCommentsURI.replace(":workflowCode", workflowCode).replace(":actionCode", workflowActionCode));
		return this.workflowActionCommentsRESTService.get(payload);
	}
	...
	public getWorkflowInboxTasks(payload: Pageable): angular.IPromise<Page<WorkflowTask>> {
		return this.workflowInboxTasksRESTService.get(payload);
	}
	...
	public getWorkflowEditableItems(itemUids: string[]): angular.IPromise<WorkflowEditableItem[]> {
		return this.workflowEditableItemsRESTService.get({
			itemUids
	...
	public openPageWorkflowMenu() {
		this.sharedDataService.get(this.OPEN_PAGE_WORKFLOW_MENU).then((data: string) => {
			if (data === OpenPageWorkflowMenu.Default) {
				this.perspectiveService.getActivePerspectiveKey().then((activePerspective: string) => {
					if (activePerspective === CMSModesService.BASIC_PERSPECTIVE_KEY ||
						...
						this.sharedDataService.set(this.OPEN_PAGE_WORKFLOW_MENU, OpenPageWorkflowMenu.SwitchPerspective).then(() => {
	...
	public loadExperienceAndOpenPageWorkflowMenu(task: WorkflowTask) {
		if (task) {
			this.catalogService.getDefaultSiteForContentCatalog(task.attachments[0].catalogId).then((defaultSite: ISite) => {
				...
				if (!!this.windowUtils.getTargetIFrame()) {
					this.experienceService.compareWithCurrentExperience(experienceParams).then((isEqual: boolean) => {
						if (isEqual) {
							this.sharedDataService.set(this.OPEN_PAGE_WORKFLOW_MENU, OpenPageWorkflowMenu.Default).then(() => {
