...
import {CrossFrameEventService, IAlertServiceType, ISeComponent, IUriContext, SeComponent, TypedMap} from "smarteditcommons";
...
export class PageSynchronizationPanel implements ISeComponent {
	...
	private SYNCHRONIZATION_PAGE_SELECT_ALL_SLOTS_LABEL = "se.cms.synchronization.page.select.all.slots";
	...
	public $onInit() {
		this.removeOnPageUpdateEvtHandler = this.crossFrameEventService.subscribe(this.EVENTS.PAGE_UPDATED, this.evaluateIfSyncIsApproved.bind(this));
	}
	...
	public getSyncStatus = (): angular.IPromise<ISyncStatus> => {
		return this.$q.all([
			...
			(this.syncStatus as any).selectAll = this.SYNCHRONIZATION_PAGE_SELECT_ALL_SLOTS_LABEL;
	...
	private evaluateIfSyncIsApproved(): angular.IPromise<void> {
		return this.pageService.isPageApproved(this.cmsPage.uuid).then((isPageApproved: boolean) => {
			if (!isPageApproved) {
