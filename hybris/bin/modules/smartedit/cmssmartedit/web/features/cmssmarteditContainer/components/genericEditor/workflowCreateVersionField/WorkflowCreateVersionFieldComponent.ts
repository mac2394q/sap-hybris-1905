...
import {WorkflowAction, WorkflowDecision, WorkflowTemplate} from 'cmssmarteditcontainer/components/workflow/dtos';
...
export class WorkflowCreateVersionFieldComponent implements ISeComponent {
	...
	public onCreateVersionChange(): void {
		if (this.model.createVersion === false) {
			...
			if (this.model.decisionCode && this.lodash.isEmpty(this.model.versionLabel)) {
				this.workflowService.getAllActionsForWorkflowCode(this.model.workflowCode).then((workflowActions: WorkflowAction[]) => {
					const workflowAction = workflowActions.find((action: WorkflowAction) => {
						return action.code === this.model.actionCode;
					});
					const workflowDecision = workflowAction.decisions.find((decision: WorkflowDecision) => {
						return decision.code === this.model.decisionCode;
					});
					this.model.versionLabel = workflowDecision.name + " for " + workflowAction.name;
				});
				// Create version label generation while creating workflows
			} else {
				this.workflowService.getWorkflowTemplateByCode(this.model.templateCode).then((workflow: WorkflowTemplate) => {
	...
	$onInit(): void {
		this.unregisterOnChangeEvent = this.editor.api.addContentChangeEvent(this.onCreateVersionChange.bind(this));
	}
