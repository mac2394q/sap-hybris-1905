 ...
 */
import {GenericEditorField, IAlertService, IPageInfoService, Page, Pageable, SeInjectable, SystemEventService, TypedMap} from 'smarteditcommons';
import {Workflow, WorkflowAction, WorkflowActionComment, WorkflowDecision, WorkflowOperations, WorkflowStatus, WorkflowTask, WorkflowTemplate} from 'cmssmarteditcontainer/components/workflow/dtos';
...
export class WorkflowFacade {
	...
	public startWorkflow(): angular.IPromise<Workflow> {
		return this._getWorkflowDataForEditor().then((componentData: any) => {
			const unregisterErrorListener = this.systemEventService.subscribe(
				this.GENERIC_EDITOR_UNRELATED_VALIDATION_MESSAGES_EVENT, this._handleUnrelatedValidationErrors.bind(this));
	...
	public getCommentsForWorkflowAction(workflowCode: string, workflowActionCode: string, payload: Pageable): angular.IPromise<Page<WorkflowActionComment>> {
		return this.workflowService.getCommentsForWorkflowAction(workflowCode, workflowActionCode, payload).then((response: Page<WorkflowActionComment>) => {
	...
	public getWorkflowInboxTasks(payload: Pageable): angular.IPromise<Page<WorkflowTask>> {
		return this.workflowService.getWorkflowInboxTasks(payload).then((page: Page<WorkflowTask>) => {
	 ...
	 * @returns {angular.IPromise<void>} A promise that resolves to void.
	 */
	public makeDecision(workflowCode: string, workflowAction: WorkflowAction, decision: WorkflowDecision): angular.IPromise<Workflow> {
		const componentData = {
			title: decision.name,
			structure: {
		...
		return this.genericEditorModalService.open(componentData, (result: Workflow) => {
			...
			return this.pageInfoService.getPageUUID().then((currentPageUuid: string) => {
				return this.systemEventService.publish(this.SYNCHRONIZATION_POLLING.SPEED_UP, currentPageUuid).then(() => {
					...
					this.systemEventService.publish(this.SYNCHRONIZATION_POLLING.SLOW_DOWN, currentPageUuid);
					return result;
				});
			});
		}, null);
	...
	private _getWorkflowDataForEditor(workflow: Workflow = null): angular.IPromise<any> {
		return this.pageInfoService.getPageUUID().then((pageUuid: string) => {
			...
			if (isStartingWorkflow) {
				const templateCodeField = this.startWorkflowStructureAttributes.find((structureAttribute: GenericEditorField) => {
					return structureAttribute.qualifier === "templateCode";
				});
				return this.workflowService.getWorkflowTemplates({}).then((workflowTemplates: WorkflowTemplate[]) => {
	...
	private _handleUnrelatedValidationErrors(key: string, eventData: any): void {
		if (eventData.sourceGenericEditorId) {
			...
			if (errorsToRepublish.length > 0) {
				this.systemEventService.publishAsync(this.GENERIC_EDITOR_UNRELATED_VALIDATION_MESSAGES_EVENT, {
