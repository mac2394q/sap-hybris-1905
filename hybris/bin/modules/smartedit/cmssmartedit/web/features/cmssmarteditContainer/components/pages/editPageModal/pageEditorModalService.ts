...
export class PageEditorModalService {
	 ...
	 */
	private _applyReadOnlyMode(page: ICMSPage, config: IGenericEditorModalServiceComponent): angular.IPromise<void> {
		if (!!page.uid) {
			return this.contextAwareEditableItemService.isItemEditable(page.uid).then((isEditable) => {
				config.readOnlyMode = !isEditable;
				if (config.readOnlyMode) {
					config.messages = [{
						type: "info",
						message: this.$translate.instant("se.cms.pageeditormodal.page.readonly.message")
					}];
				}
			});
		}
