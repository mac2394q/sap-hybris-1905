...
import {Workflow, WorkflowAction, WorkflowActionComment, WorkflowActionStatus, WorkflowDecision} from '../../dtos';
...
export class WorkflowActionItemComponent implements ISeComponent {
	...
	constructor(
		...
		this.pageSize = 10;
		this.unregWorkflowMenuOpenedEvent = this.systemEventService.subscribe('WORKFLOW_ITEM_MENU_OPENED_EVENT', this.onOtherMenuOpening.bind(this));
	...
	public loadComments = (mask: string, pageSize: number, currentPage: number): angular.IPromise<Page<WorkflowActionComment>> => {
		const payload: Pageable = {
			currentPage,
			mask,
			pageSize
		};
		return this.workflowFacade.getCommentsForWorkflowAction(this.workflow.workflowCode, this.workflowAction.code, payload).then((page: Page<WorkflowActionComment>) => {
	...
	public getReadableStatus = (): string => {
		if (this.workflowAction.status.toLowerCase() === WorkflowActionStatus.IN_PROGRESS) {
			...
			return this.$translate.instant('se.cms.actionitem.page.workflow.action.status.not.started');
		}
	...
	public displayDecisionsButtons = (): boolean => {
		...
		return this.canMakeDecisions && this.workflowAction.isCurrentUserParticipant && decisions.length > 0;
	}
	...
	public showComments = (): boolean => {
		const isContainerExpanded = this.collapsibleContainerApi.isExpanded();
