...
import {CrossFrameEventService, IRestService, IRestServiceFactory, Pagination, SeInjectable} from 'smarteditcommons';
...
export class WorkflowTasksPollingService {
	...
	constructor(
		...
		this.crossFrameEventService.subscribe(this.EVENTS.AUTHORIZATION_SUCCESS, this.startPolling.bind(this));
		this.crossFrameEventService.subscribe(this.EVENTS.LOGOUT, this.stopPolling.bind(this));
		this.crossFrameEventService.subscribe(this.EVENTS.REAUTH_STARTED, this.stopPolling.bind(this));
		this._initPolling();
	}
	...
	private _initPolling() {
		this.syncPollingTimer = this.timerService.createTimer(this._fetchInboxTasks.bind(this, false), INBOX_POLLING_PARAMS.INBOX_POLLING_TIMEOUT);
