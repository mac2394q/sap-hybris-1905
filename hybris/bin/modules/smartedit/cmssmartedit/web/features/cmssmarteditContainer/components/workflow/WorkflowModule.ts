...
import {CrossFrameEventService, INotificationService, IPageInfoService, SeModule, TypedMap} from 'smarteditcommons';
	...
	initialize: (
		...
		crossFrameEventService.subscribe(EVENTS.PAGE_CHANGE, () => {
			...
			pageInfoService.getPageUUID().then((pageUuid: string) => {
				workflowService.getActiveWorkflowForPageUuid(pageUuid).then((workflow: Workflow) => {
