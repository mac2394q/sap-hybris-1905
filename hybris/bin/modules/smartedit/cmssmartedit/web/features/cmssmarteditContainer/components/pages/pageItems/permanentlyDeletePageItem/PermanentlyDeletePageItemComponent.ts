 ...
 */
import {CrossFrameEventService, ICatalogService, ISeComponent, IUriContext, MultiNamePermissionContext, SeComponent} from 'smarteditcommons';
...
export class PermanentlyDeletePageItemComponent implements ISeComponent {
	...
	fetchPageDeletableConditions(): void {
		this.catalogService.retrieveUriContext().then((uriContext: IUriContext) => {
			this.catalogService.getContentCatalogActiveVersion(this.uriContext).then((activeVersion: string) => {
				this.cmsitemsRestService.get({
					...
					this.isDeletable = result.pagination.totalCount === 0 || result.response[0].pageStatus === CMSPageStatus.DELETED;
				});
			});
		});
	}
