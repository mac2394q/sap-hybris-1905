 ...
 */
import {ICatalogService, ISeComponent, IUriContext, IUrlService, SeComponent, SystemEventService} from 'smarteditcommons';
...
export class TrashLinkComponent implements ISeComponent {
	...
	$onInit() {
		...
		this.unsibscribeContentCatalogUpdateEvent = this.systemEventService.subscribe(this.EVENT_CONTENT_CATALOG_UPDATE, this.updateTrashedPagesCount.bind(this));
	}
     ...
     */
	updateTrashedPagesCount(): angular.IPromise<void> {
		return this.managePageService.getSoftDeletedPagesCount(this.uriContext).then((trashedPagesCount) => {
