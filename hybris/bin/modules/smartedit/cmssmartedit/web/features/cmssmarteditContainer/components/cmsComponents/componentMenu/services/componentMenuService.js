 ...
 */
angular.module('componentMenuServiceModule', ['smarteditServicesModule', 'yLoDashModule'])
    /**
     * This service provides functionality to the component menu; it's meant to be used internally. Thus, no ng-docs are added.
     */
    .service('componentMenuService', function($timeout, storageService, catalogService, experienceService, lodash) {
        ...
        this.getInitialCatalogVersion = function(catalogVersions) {
            return storageService.getValueFromLocalStorage(SELECTED_CATALOG_VERSION_COOKIE_NAME).then(function(rawValue) {
        ...
        this.persistCatalogVersion = function(catalogVersionId) {
            storageService._setValueInLocalStorage(SELECTED_CATALOG_VERSION_COOKIE_NAME, catalogVersionId);
        };
    });
