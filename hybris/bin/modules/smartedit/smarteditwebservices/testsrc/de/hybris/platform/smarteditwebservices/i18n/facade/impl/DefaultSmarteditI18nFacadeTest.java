...
public class DefaultSmarteditI18nFacadeTest
{
	...
	{
		// GIVEN
		doReturn(currentTenant).when(smarteditI18nFacade).getCurrentTenant();
		doReturn(true).when(smarteditI18nFacade).isSystemInitialized();
		doNothing().when(smarteditI18nFacade).initializeLocalizations();

		// WHEN
		smarteditI18nFacade.afterPropertiesSet();
		smarteditI18nFacade.getTenantListener().afterTenantStartUp(currentTenant);

		// THEN
		verify(smarteditI18nFacade).initializeLocalizations();
	}
	...
	public void shouldNotGetLocalizationsWhen_SystemInitialized_And_TenantOnStartupDoesNotMatchCurrentTenant()
	{
		...
		// WHEN
		smarteditI18nFacade.afterPropertiesSet();
		smarteditI18nFacade.getTenantListener().afterTenantStartUp(anyTenant);

		// THEN
		verify(smarteditI18nFacade, never()).initializeLocalizations();
	}
	...
	public void shouldNotGetLocalizationsWhen_SystemIsNotInitialized_And_TenantOnStartupMatchCurrentTenant()
	{
		// GIVEN
		doReturn(currentTenant).when(smarteditI18nFacade).getCurrentTenant();
		doReturn(false).when(smarteditI18nFacade).isSystemInitialized();
