...
public class AfterOrderSaveEventHandler extends DefaultSaveEventHandler
{
	...
	public void handleEvent(final AfterSaveEvent event)
	{
				...
				&& (getModelService().get(pk) instanceof OrderModel))
		{
			...
			{
				LOGGER.error("Failed to send order data to websocket", e);
			}
			try
			{
	...
	{
		return (event.getType() == AfterSaveEvent.CREATE) || (event.getType() == AfterSaveEvent.UPDATE) && getConfigurationService().getConfiguration().getBoolean(EventpublisherConstants.WEBSOCKET_ENABLED,true);
