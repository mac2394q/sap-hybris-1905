 ...
 */
package com.sap.hybris.cec.sso.controller;
...
public class JWTRedirectController
{
	...
	private static final Logger LOGGER = Logger.getLogger(JWTRedirectController.class);
	...
	public String redirect(final HttpServletResponse response, final HttpServletRequest request)
	{
		...
		try
		{
			...
			final String redirectURL = StringUtils.defaultIfEmpty(Config.getParameter(CecssoConstants.SSO_REDIRECT_URL),
					CecssoConstants.DEFAULT_REDIRECT_URL);

			return CecssoConstants.REDIRECT_PREFIX + redirectURL + referenceURL;
	...
	public void storeTokenFromJWT(final HttpServletResponse response, final UserModel user)
	{
		try
		{
			...
			final String cookiePath = StringUtils.defaultIfEmpty(Config.getParameter(CecssoConstants.SSO_COOKIE_PATH),
					CecssoConstants.DEFAULT_COOKIE_PATH);

			final String cookieMaxAgeStr = StringUtils.defaultIfEmpty(Config.getParameter(CecssoConstants.SSO_COOKIE_MAX_AGE),
					String.valueOf(CecssoConstants.DEFAULT_COOKIE_MAX_AGE));

			int cookieMaxAge;

			if (!NumberUtils.isNumber(cookieMaxAgeStr))
			{
				cookieMaxAge = CecssoConstants.DEFAULT_COOKIE_MAX_AGE;
			}
			else
			{
			...
			UserManager.getInstance().storeLoginTokenCookie(
					//
					StringUtils.defaultIfEmpty(Config.getParameter(CecssoConstants.SSO_COOKIE_NAME),
							CecssoConstants.SSO_DEFAULT_COOKIE_NAME), // cookie name
					user.getUid(), // user id
					"en", // language iso code
					null, // credentials to check later
					cookiePath, // cookie path
					StringUtils.defaultIfEmpty(Config.getParameter(CecssoConstants.SSO_COOKIE_DOMAIN),
							CecssoConstants.SSO_DEFAULT_COOKIE_DOMAIN), // cookie domain
					true, // secure cookie
