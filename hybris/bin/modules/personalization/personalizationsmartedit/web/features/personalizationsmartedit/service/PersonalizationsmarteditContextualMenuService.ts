...
export class PersonalizationsmarteditContextualMenuService {
	...
	isEditPersnalizationInWorkflowAllowed(condition: boolean): boolean {
		...
		if (isEditPersonalizationInWorkflowPropertyEnabled) {
			...
			return condition && !this.isWorkflowRunning;
		}
	}
	...
	isContextualMenuAddItemEnabled(config: any): boolean {
		...
		return this.isEditPersnalizationInWorkflowAllowed(isEnabled);
	}
