...
export class PersonalizationsmarteditManagerViewController {
	...
	constructor(
		...
		this.init = () => {
			this.systemEventService.registerEventHandler('CUSTOMIZATIONS_MODIFIED', () => {
				this.refreshGrid();
				return this.$q.when();
			});
		};
	...
	public getElementToScroll = () => {
		return this.$scope.scrollZoneElement;
	}
	...
	public isSearchGridHeaderHidden = () => {
		return this.$scope.scrollZoneElement.scrollTop() >= 120;
	}

	public scrollZoneReturnToTop = () => {
		this.$scope.scrollZoneElement.animate({
			scrollTop: 0
		}, 500);
	}

	public isReturnToTopButtonVisible = () => {
		return this.$scope.scrollZoneElement.scrollTop() > 50;
	}
	...
	private isScrollZoneVisible = () => {
		return this.$scope.scrollZoneElement.get(0).scrollHeight > this.$scope.scrollZoneElement.get(0).clientHeight;
	}

	private getCustomizations = (filter: any) => {
		this.personalizationsmarteditManagerViewUtilsService.getCustomizations(filter)
			.then((response: any) => {
				Array.prototype.push.apply(this.customizations, response.customizations);
				...
				this.personalizationsmarteditMessageHandler.sendError(this.$filter('translate')('personalization.error.gettingcustomizations'));
				this.moreCustomizationsRequestProcessing = false;
			});
	}
