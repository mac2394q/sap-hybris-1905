	...
	class PersonalizationsmarteditCommerceCustomizationViewController {
		...
		private populateActions = () => {
			this.personalizationsmarteditRestService.getActions(this.$scope.customization.code, this.$scope.variation.code, {})
				.then((response: any) => {
					...
					this.actionsDataFactory.resetActions();
					Array.prototype.push.apply(this.actionsDataFactory.getActions(), actions);
				}, () => {
		...
		private deleteActions = (customizationCode: any, variationCode: any, deleteData: any) => {
			...
			this.personalizationsmarteditRestService.deleteActions(customizationCode, variationCode, deleteData, {})
				.then((response: any) => {
					this.personalizationsmarteditMessageHandler.sendSuccess(this.$filter('translate')('personalization.info.removingaction'));
					this.sendRefreshEvent();
					deferred.resolve(response);
				}, () => {
					this.personalizationsmarteditMessageHandler.sendError(this.$filter('translate')('personalization.error.removingaction'));
					deferred.resolve();
				});
			return deferred.promise;
		}
			...
			updateTypes.forEach((type: any) => {
				if (type.updateActions) {
					...
					type.updateActions(customizationCode, variationCode, actionsForType, respCreate, respDelete)
						.then(() => {
							this.personalizationsmarteditMessageHandler.sendSuccess(this.$filter('translate')('personalization.info.updatingactions'));
							this.sendRefreshEvent();
						}, () => {
							this.personalizationsmarteditMessageHandler.sendError(this.$filter('translate')('personalization.error.updatingactions'));
						});

				} else {
					this.$log.debug(this.$filter('translate')('personalization.error.noupdatingactions'));
				}
			});
		}
