...
public class KymaEventEmitTaskRunner implements TaskRunner<TaskModel>
{
	...
	public void run(final TaskService taskService, final TaskModel task)
	{
		...
		try
		{
			...
			LOG.info(String.format("Event (EventId : %s) was sent. Response: %s", publishRequestData.getEventId(), response));
		}
	...
	protected void scheduleEventEmitTask(final Message message, final PublishRequestData publishRequestData, final Integer retry)
	{
		...
		if (retry >= Config.getInt(MAX_RETRIES, 3))
		{
			...
			final String url, final AbstractDestinationModel destination)
	{
		...
		for (int i = 0; i < maxConsecutiveRetries; i++)
		{
			try
			{
				...
				LOG.info(String.format("Consecutive attempt (Number : %d), Event (EventId : %s) sending response : %s", i + 2,
			...
			catch (final ResourceAccessException e)
			{
				...
				LOG.error(String.format("Consecutive attempt failed (Number : %d), Event (EventId : %s), cause: %s", i + 2,
