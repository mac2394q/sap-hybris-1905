...
public class KymaUpdateConsumedDestinationsJobPerformable extends AbstractJobPerformable<CronJobModel>
{
	...
	public PerformResult perform(final CronJobModel cronJob)
	{
		...
		if (cronJob.getCreationtime().before(DateTime.now().minusMinutes(Config.getInt(CRONJOB_LIFESPAN_PROP_KEY, 20)).toDate())
		...
		try
		{
			...
			if (!validateInfoData(infoData))
			{
				...
				{
					destinationTarget.setRegistrationStatusInfo(String.format("Invalid response from {%s} endpoint", getInfoDestinationId));
					getModelService().save(destinationTarget);
				}
				else
				{
			...
			//clear status field
			destinationTarget.setRegistrationStatusInfo(StringUtils.EMPTY);
			getModelService().save(destinationTarget);
