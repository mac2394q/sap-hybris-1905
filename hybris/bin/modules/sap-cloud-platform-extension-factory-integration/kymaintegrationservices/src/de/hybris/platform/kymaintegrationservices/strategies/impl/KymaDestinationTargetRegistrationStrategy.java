...
public class KymaDestinationTargetRegistrationStrategy implements DestinationTargetRegistrationStrategy
{
			...
			throws ApiRegistrationException
	{
		...
		for (final ExposedDestinationModel exposedDestination : exposedDestinations)
		{
			...
			{
				LOG.error("Registration is failed for the destination: {}" + destinationTarget.getId(), e);
	...
	protected void startGetInfoCronJob(final DestinationTargetModel destinationTarget)
	{
		...
		period = (period > 0 && period < 60) ? period : 5;

		int initialDelay = Config.getInt(KYMA_GETINFO_CRONJOB_TIME_INITIAL_DELAY, DEFAULT_KYMA_GETINFO_CRONJOB_TIME_INITIAL_DELAY);
		initialDelay = (initialDelay > 0 && initialDelay <= 60) ? initialDelay : 5;
