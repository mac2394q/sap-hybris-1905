...
public class DefaultSynchronizationFacadeTest
{
	...
	@Mock
	private SyncItemInfo syncItemInfo1;
	@Mock
	private SyncItemInfo syncItemInfo2;
	...
	{
		// given
		final ItemModel firstItem = mockItemModelWithPK(21L, ItemModel.class);
		final ItemModel secondItem = mockItemModelWithPK(22L, ItemModel.class);
		...
		item1WithRelatedItems.add(firstItem);
		when(relatedItemsCollector.collect(eq(firstItem), anyMap())).thenReturn(item1WithRelatedItems);
		when(relatedItemsCollector.collect(eq(secondItem), anyMap())).thenReturn(Lists.newArrayList(secondItem));

		final List<ItemModel> itemGroup = new ArrayList<>(item1RelatedItems);
		itemGroup.add(firstItem);
		itemGroup.add(secondItem);
		for (final ItemModel item : item1RelatedItems)
		{
			when(synchronizationStatusService.getSyncInfo(item, Lists.newArrayList(jobA, jobB))).thenReturn(Collections.singletonList(syncItemInfo1));
		}
		when(syncItemInfo1.getSyncStatus()).thenReturn(SyncItemStatus.IN_SYNC);

		// when
		final Optional<Boolean> inSync = facade.isInSync(Lists.newArrayList(firstItem, secondItem), syncJob, Collections.emptyMap());
	...
	{
		final ItemModel item = mockItemModelWithPK(21L, ItemModel.class);
		final List<ItemModel> item1RelatedItems = mockItemsWithPK(10, 200, ItemModel.class);
		final List<ItemModel> item1WithRelatedItems = mockRelatedItemsFor(item, item1RelatedItems);

		mockFacadeInOutSynchronizationForItems(Lists.newArrayList(jobA), Lists.newArrayList(jobB), item1WithRelatedItems);
		doReturn(Boolean.TRUE).when(synchronizationStatusService).matchesSyncStatus(any(), any(), any());

		assertThat(facade.isInSync(item, new HashMap<>()).get()).isTrue();
	}
	...
	public void testItemIsInSyncItemIsNew()
	{
		...
		doReturn(Boolean.TRUE).when(synchronizationStatusService).matchesSyncStatus(item1WithRelatedItems,
				Lists.newArrayList(jobA), SyncItemStatus.IN_SYNC);
		doReturn(Boolean.TRUE).when(synchronizationStatusService).matchesSyncStatus(item1WithRelatedItems,
				Lists.newArrayList(jobB), SyncItemStatus.IN_SYNC);
		doReturn(Boolean.TRUE).when(modelService).isNew(item);

		assertThat(facade.isInSync(item, new HashMap<>()).isPresent()).isFalse();
	}
	...
	public void testSyncStatusNotAvailable()
	{
		...
		doReturn(Boolean.FALSE).when(catalogTypeService).isCatalogVersionAwareModel(item);
	...
	public void testItemIsInSyncFalse()
	{
		...
		mockFacadeInOutSynchronizationForItems(Lists.newArrayList(jobA), Lists.newArrayList(jobB), item1WithRelatedItems);
		when(synchronizationStatusService.getSyncInfo(item1RelatedItems.get(0), Lists.newArrayList(jobA, jobB))).thenReturn(Collections.singletonList(syncItemInfo1));
		when(syncItemInfo1.getSyncStatus()).thenReturn(SyncItemStatus.NOT_SYNC);

		assertThat(facade.isInSync(item, new HashMap<>()).get()).isFalse();
	}

	@Test
	public void testJobIsApplicableForItems()
	{
		//given
		final List<ItemModel> items = mockItemsWithPK(10, 200, ItemModel.class);
		for (final ItemModel item : items)
		{
			when(syncJobApplicableTypesStrategy.checkIfApplicable(item, jobA)).thenReturn(true);
		}

		//when
		final boolean isJobApplicable = facade.isApplicableForItems(jobA, items);

		//then
		assertThat(isJobApplicable).isTrue();
	}

	@Test
	public void testJobIsNotApplicableForItemsIfItIsNotApplicableForOneOfThem()
	{
		//given
		final List<ItemModel> items = mockItemsWithPK(10, 200, ItemModel.class);
		when(syncJobApplicableTypesStrategy.checkIfApplicable(items.get(0), jobA)).thenReturn(false);
		for (final ItemModel item : items.subList(1, items.size()-1))
		{
			when(syncJobApplicableTypesStrategy.checkIfApplicable(item, jobA)).thenReturn(true);
		}
		//when
		...
		//then
		assertThat(isJobApplicable).isFalse();
	}
	...
	public void testItemsCatalogVersionSync()
	{
		...
		//then
		assertThat(itemsCatalogVersions).containsOnlyElementsOf(catalogs);
	}

	@Test
	public void testIsInSyncWhenSyncStatusIsInSync()
	{
		//given
		final List<ItemModel> items = mockEmptyRelatedItemsFor(item1);
		when(synchronizationStatusService.getSyncInfo(item1, Collections.singletonList(jobA)))
				.thenReturn(Collections.singletonList(syncItemInfo1));
		when(syncItemInfo1.getSyncStatus()).thenReturn(SyncItemStatus.IN_SYNC);

		//when
		final Optional<Boolean> isInSync = facade.isInSync(items, jobA, new HashMap<>());

		//then
		assertThat(isInSync.get()).isTrue();
	}

	@Test
	public void testIsNotInSyncWhenSyncStatusIsNotSync()
	{
		//given
		final List<ItemModel> items = mockEmptyRelatedItemsFor(item1);
		when(synchronizationStatusService.getSyncInfo(item1, Collections.singletonList(jobA)))
		...
		//then
		assertThat(isInSync.get()).isFalse();
	}

	@Test
	public void testIsNotInSyncWhenSyncStatusIsNotApplicable()
	{
		//given
		final List<ItemModel> items = mockEmptyRelatedItemsFor(item1);
		when(synchronizationStatusService.getSyncInfo(item1, Collections.singletonList(jobA)))
				.thenReturn(Collections.singletonList(syncItemInfo1));
		when(syncItemInfo1.getSyncStatus()).thenReturn(SyncItemStatus.NOT_APPLICABLE);
	...
	@Test
	public void testIsInSyncWhenSyncStatusesAreInSyncAndNotApplicable()
	{
		//given
		final List<ItemModel> items = mockEmptyRelatedItemsFor(item1);
		when(synchronizationStatusService.getSyncInfo(item1, Collections.singletonList(jobA)))
				.thenReturn(Arrays.asList(syncItemInfo1, syncItemInfo2));
		when(syncItemInfo1.getSyncStatus()).thenReturn(SyncItemStatus.NOT_APPLICABLE);
		when(syncItemInfo2.getSyncStatus()).thenReturn(SyncItemStatus.IN_SYNC);
	...
	public void testPartialSyncInfo()
	{
		...
		when(syncJobApplicableTypesStrategy.checkIfApplicable(any(), any())).thenReturn(true);
	...
	@Test
	public void testPartialSyncInfoIsInSyncEvenIfNotApplicableJobsAreNotInSync()
	{
		//given
		final ItemModel product = mock(ProductModel.class);
		final ItemModel thumbnail = mock(MediaModel.class);
		...
		when(syncJobApplicableTypesStrategy.checkIfApplicable(product, inSyncJob)).thenReturn(true);
		when(syncJobApplicableTypesStrategy.checkIfApplicable(thumbnail, inSyncJob)).thenReturn(true);
		when(syncJobApplicableTypesStrategy.checkIfApplicable(product, notInSyncJob)).thenReturn(false);
		when(syncJobApplicableTypesStrategy.checkIfApplicable(thumbnail, notInSyncJob)).thenReturn(true);
		...
		final PartialSyncInfo partialSyncInfo = info.get();
		assertThat(partialSyncInfo.getInboundSyncStatus()).hasSize(1);
		assertThat(partialSyncInfo.getInboundSyncStatus().get(inSyncJob)).isTrue();
		assertThat(partialSyncInfo.getInboundSyncStatus().get(notInSyncJob)).isNull();

		assertThat(partialSyncInfo.getOutboundSyncStatus()).hasSize(1);
		assertThat(partialSyncInfo.getOutboundSyncStatus().get(inSyncJob)).isTrue();
		assertThat(partialSyncInfo.getOutboundSyncStatus().get(notInSyncJob)).isNull();
	}
	...
	public void testOnlyItemsSyncRun()
	{
		...
		syncTask.setParameter(RelatedItemsCollector.MAX_RECURSION_DEPTH, recursionDepth);
		// when
	...
	public void testCronJobRerun()
	{
		...
		final long pkNumber = 100L;
		doReturn(PK.fromLong(pkNumber)).when(newScheduledMediaModel).getPk();
