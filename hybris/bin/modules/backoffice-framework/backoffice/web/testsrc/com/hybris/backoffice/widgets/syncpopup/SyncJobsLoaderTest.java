...
public class SyncJobsLoaderTest
{
	...
	public void testGetSyncJobs()
	{
		...
		when(synchronizationFacade.isApplicableForItems(job1, selectedItems)).thenReturn(true);
		when(synchronizationFacade.isApplicableForItems(job2, selectedItems)).thenReturn(true);
	...
	@Test
	public void testFilterOutNonApplicableJobs()
	{
		final ItemModel item1 = mock(ItemModel.class);
		final ItemModel item2 = mock(ItemModel.class);
		final List<ItemModel> selectedItems = Lists.newArrayList(item1, item2);
		...
		final SyncItemJobModel job1 = mock(SyncItemJobModel.class);
		final SyncItemJobModel job2 = mock(SyncItemJobModel.class);
		final SyncItemJobModel job3 = mock(SyncItemJobModel.class);
		final SyncItemJobModel job4 = mock(SyncItemJobModel.class);

		when(synchronizationFacade.getCatalogVersionAwareItems(selectedItems)).thenReturn(Lists.newArrayList(item1, item2));
		when(synchronizationFacade.getSyncCatalogVersion(selectedItems)).thenReturn(Optional.of(cv1));
		when(synchronizationFacade.getInboundSynchronizations(cv1)).thenReturn(Lists.newArrayList(job1, job2));
		when(synchronizationFacade.getOutboundSynchronizations(cv1)).thenReturn(Lists.newArrayList(job3, job4));
		when(synchronizationFacade.canSync(job1)).thenReturn(true);
		when(synchronizationFacade.canSync(job2)).thenReturn(true);
		when(synchronizationFacade.canSync(job3)).thenReturn(true);
		when(synchronizationFacade.canSync(job4)).thenReturn(true);

		when(synchronizationFacade.isApplicableForItems(job1, selectedItems)).thenReturn(true);
		when(synchronizationFacade.isApplicableForItems(job2, selectedItems)).thenReturn(false);
		when(synchronizationFacade.isApplicableForItems(job3, selectedItems)).thenReturn(false);
		when(synchronizationFacade.isApplicableForItems(job4, selectedItems)).thenReturn(true);

		final SyncJobsLoader syncJobsLoader = createSyncLoader(selectedItems);

		assertThat(syncJobsLoader.getLoadingStatus()).isEqualTo(SyncJobsLoader.SyncJobsLoadingStatus.OK);
		assertThat(syncJobsLoader.getPullJobs()).containsOnly(job1);
		assertThat(syncJobsLoader.getPushJobs()).containsOnly(job4);
	}
