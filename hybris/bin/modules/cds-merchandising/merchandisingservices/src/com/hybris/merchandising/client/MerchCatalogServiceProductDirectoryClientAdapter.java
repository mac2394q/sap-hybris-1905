...
public class MerchCatalogServiceProductDirectoryClientAdapter implements MerchCatalogServiceProductDirectoryClient
{
	...
	{
		return createProductDirectoryAsync(productDirectory).subscribeOn(scheduler).toBlocking().first();
	}
	...
	{
		return updateProductDirectoryAsync(productDirectoryId, productDirectory).subscribeOn(scheduler).toBlocking()
				.firstOrDefault(null);
	}
	...
	public Void deleteProductDirectory(final String productDirectoryId) {
		try {
			return deleteProductDirectoryAsync(productDirectoryId).subscribeOn(scheduler)
					.toBlocking().firstOrDefault(null);
		}
