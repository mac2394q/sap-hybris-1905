...
public class DefaultSelectiveCartDaoTest extends ServicelayerTransactionalTest
{
	...
	@Test
	public void testFindSelectiveCartByUserWithNullSavedCart()
	{
		final Optional<CartModel> optionalCart = selectiveCartDao.findSelectiveCartByUser(customer);
		Assert.assertFalse(optionalCart.isPresent());
	}

	@Test
	public void testFindSelectiveCartByUser()
	{
		cart.setVisible(Boolean.FALSE);
		modelService.save(cart);
		final Optional<CartModel> optionalCart = selectiveCartDao.findSelectiveCartByUser(customer);
		Assert.assertTrue(optionalCart.isPresent());
		Assert.assertEquals(optionalCart.get().getVisible(), Boolean.FALSE);
	}

	@Test
	public void testFindSelectiveCartByUserAndCode()
	{
		cart.setVisible(Boolean.FALSE);
		modelService.save(cart);
		final Optional<CartModel> optionalCart = selectiveCartDao.findSelectiveCartByCode(CART_UID);
		Assert.assertTrue(optionalCart.isPresent());
		Assert.assertEquals(optionalCart.get().getVisible(), Boolean.FALSE);
        }
}
