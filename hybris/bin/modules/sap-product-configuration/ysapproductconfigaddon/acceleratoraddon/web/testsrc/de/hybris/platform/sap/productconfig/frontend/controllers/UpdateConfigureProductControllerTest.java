...
public class UpdateConfigureProductControllerTest extends AbstractProductConfigControllerBaseTest
{
	...
	protected UiStateHandler uiStateHandler = new UiStateHandler();
	...
	{
		UiGroupData result = classUnderTest.findFirstConflictGroupForCsticId(null, "csticId");
		assertNull(result);
	}
	...
	{
		List<UiGroupData> uiGroups = create4CsticGroups("simple.mock", "1");

		UiGroupData result = classUnderTest.findFirstConflictGroupForCsticId(uiGroups, "csticId");
		assertNull(result);
	}
	...
	{
		List<UiGroupData> uiGroups = createCsticsGroup();
		UiGroupData result = classUnderTest.findFirstConflictGroupForCsticId(uiGroups, "csticId");
		assertNull(result);
	}
	...
	public void testHandleAutoExpand_Conflict()
	{
		...
		List<CsticData> cstics = configData.getGroups().get(0).getCstics();

		UiGroupData conflictGroup = createUiConflictGroup("CONFLICT");
	...
	public void testHandleAutoExpandForceExpandTrue()
	{
		...
		UiGroupForDisplayData groupToDisplay = new UiGroupForDisplayData();
	...
	{
		String focus = "root.WCEM_STRING_SIMPLE";
	...
	{
		UiGroupData conflictGroup = createCsticsGroupWithConflicts("conflictGroup");
	...
	public void testHandleGroupIdToDisplayForNavigationLinksFocusSetNavInConflict()
	{
		...
		UiGroupData conflictGroup = createCsticsGroupWithConflicts(conflictGroupID);
		ConfigurationData configDataWithConflicts = createEmptyConfigData();
		updateData.setConfigData(configDataWithConflicts);
		List<UiGroupData> conflictsGroups = new ArrayList();
	...
	public void testGetCsticKeysForExpandedSingleLevelGroups()
	{
		...
		UiGroupData group = createCsticsGroupWithConflicts("simple.WCEM_Conflict2");
		group.setCstics(Collections.EMPTY_LIST);
		groups.add(group);
		List<String> result = classUnderTest.getCsticKeysForExpandedSingleLevelGroups(groups);
		assertNotNull(result);
