...
public class SwitchableProviderFactoryImpl implements SwitchableProviderFactory
{
	...
	{
		if (getRegisteredProviderFactories().size() > 1)
		{
			LOG.warn(String.format(
					"More than one provider factory is registered/deployed. This setUp is generally neigther recommended nor supported for productive environments. Only use it for testing / demo purpose. Currently registered beans: %s",
					getRegisteredProviderFactories().toString()));
		}

		final String defaultProviderFactoyBeanName = getFactoryBeanNameConfiguredViaProperty();
		if (null != defaultProviderFactoyBeanName)
		{
			if (getRegisteredProviderFactories().contains(defaultProviderFactoyBeanName))
			{
			...
			else
			{
				LOG.info(String.format(
						"Configured provider factory '%s' is not available, switching to default provider factory instead",
						defaultProviderFactoyBeanName));
			}
		}

		switchProviderFactory(findDefaultProviderFactory());
	}
	...
	protected String getFactoryBeanNameConfiguredViaProperty()
	{
		...
		final String defaultProviderFactoyBeanName = Registry.hasCurrentTenant()
				? Config.getParameter("sapproductconfig.providerFactoryBeanName")
				: null;
