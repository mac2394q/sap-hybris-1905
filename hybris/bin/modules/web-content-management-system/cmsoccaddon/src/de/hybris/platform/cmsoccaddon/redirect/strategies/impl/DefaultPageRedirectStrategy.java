 ...
 */
package de.hybris.platform.cmsoccaddon.redirect.strategies.impl;

import de.hybris.platform.cms2.model.preview.PreviewDataModel;
import de.hybris.platform.cmsoccaddon.redirect.strategies.PageRedirectStrategy;
...
public class DefaultPageRedirectStrategy implements PageRedirectStrategy
{
	...
	public boolean shouldRedirect(final HttpServletRequest request, final PreviewDataModel previewData)
	{
		...
		{
			return getPageRedirectSuppliers().stream().filter(supplier -> supplier.getConstrainedBy().test(previewData.getPage()))
					.findFirst().map(supplier -> supplier.shouldRedirect(request, previewData)).orElse(false);
		}
		return false;
	}
