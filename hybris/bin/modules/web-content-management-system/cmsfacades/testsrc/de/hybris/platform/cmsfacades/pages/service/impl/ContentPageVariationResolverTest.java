...
public class ContentPageVariationResolverTest
{
	...
	public void shouldFindDefaultPagesForVariationPageModel()
	{
		...
		doReturn(Arrays.asList(defaultPageModel)).when(resolver).findDefaultPages(allContentPages);
	...
	{
		doReturn(Boolean.TRUE).when(resolver).isDefaultPage(defaultPageModel);
		doReturn(Arrays.asList(variationPageModel, secondVariationPageModel, otherVariationPageModel)).when(resolver)
				.findPagesByType(defaultPageModel.getItemtype(), Boolean.FALSE);
