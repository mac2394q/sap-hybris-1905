...
public class AbstractComponentTypeFacadeIntegrationTest
{
	...
	{
		final List<ComponentTypeData> componentTypes = componentTypeFacade.getAllCmsItemTypes(Arrays.asList(StructureTypeCategory.values()), false);
	...
	{
		final ComponentTypeData categoryPageType = componentTypeFacade.getCmsItemTypeByCodeAndMode(CategoryPageModel._TYPECODE, null, false);
	...
	{
		final ComponentTypeData categoryPageType = componentTypeFacade.getCmsItemTypeByCodeAndMode(ContentPageModel._TYPECODE, null, false);
	...
	public void testAllComponentTypeStructuresArePresent() throws ComponentTypeNotFoundException
	{
		typeBluePrint.keySet().stream().forEach(typeCode -> {
			try
			{
				final ComponentTypeData componentTypeByCode = componentTypeFacade.getCmsItemTypeByCodeAndMode(typeCode, null, false);
