...
public class ContentPageModelMother extends AbstractModelMother<ContentPageModel>
{
	...
	public ContentPageModel approveHomePage(final CatalogVersionModel catalogVersion)
	{
		return approvePage(UID_HOMEPAGE, catalogVersion);
	}

	public ContentPageModel approvePage(final String uid, final CatalogVersionModel catalogVersion)
	{
		ContentPageModel contentPageModel = getContentPageDao().getByUidAndCatalogVersion(uid, catalogVersion);
		contentPageModel.setApprovalStatus(CmsApprovalStatus.APPROVED);
		getModelService().save(contentPageModel);
