...
@IntegrationTest
public class RenderingPageConverterIntegrationTest extends BaseIntegrationTest
{
	// --------------------------------------------------------------------------
	// Variables
	// --------------------------------------------------------------------------
	...
	@Resource(name = "cmsPageModelToDataRenderingConverter")
	private Converter<AbstractPageModel, AbstractPageData> cmsPageRenderingConverter;
	...
	{
		// WHEN
		final AbstractPageData pageData = cmsPageRenderingConverter.convert(contentPageModel);
	...
	protected void hasValidPageData(final AbstractPageData pageData)
	{
		...
		hasValidHeaderSlotData(getSlotById(ContentSlotModelMother.UID_HEADER, pageData.getContentSlots()));
		hasValidFooterSlotData(getSlotById(ContentSlotModelMother.UID_FOOTER, pageData.getContentSlots()));
	}

	protected PageContentSlotData getSlotById(final String slotId, final List<PageContentSlotData> slots)
	{
		return slots.stream().filter(slotData -> slotId.equals(slotData.getSlotId())).findFirst().orElse(null);
	}
	...
	protected void hasValidABTestComponentData(final AbstractCMSComponentData componentData)
	{
		...
		assertThat(nestedComponents,
				containsInAnyOrder(ParagraphComponentModelMother.UID_HEADER, ParagraphComponentModelMother.UID_FOOTER));
	}
