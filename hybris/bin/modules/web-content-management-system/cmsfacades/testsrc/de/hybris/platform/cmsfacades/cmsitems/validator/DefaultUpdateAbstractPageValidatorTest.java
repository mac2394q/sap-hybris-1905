...
public class DefaultUpdateAbstractPageValidatorTest
{
	...
	public void setup()
	{
		...
		// Workflows
		when(cmsWorkflowService.isAnyItemInWorkflow(Arrays.asList(pageUnderTest))).thenReturn(false);
	}
	...
	@Test
	public void givenPage_WhenValidated_ThenShouldRunWorkflowItemValidator()
	{
		// WHEN
		validator.validate(pageUnderTest);

		// THEN
		verify(workflowItemValidator).validate(pageUnderTest);
	}

	@Test
	public void givenPageThatMustBeDeletedAndInWorkflow_WhenValidated_ThenItMustFail()
	{
		// GIVEN
		when(pageUnderTest.getPageStatus()).thenReturn(CmsPageStatus.DELETED);
		when(cmsWorkflowService.isAnyItemInWorkflow(Arrays.asList(pageUnderTest))).thenReturn(true);

		// WHEN
		validator.validate(pageUnderTest);

		// THEN
		assertHasError(AbstractPageModel.TYPECODE, PAGE_IN_WORKFLOW_CANNOT_BE_TRASHED, null);
	}
