...
public class CMSItemNameExistsPredicateTest
{
	...
	public void whenItemIsPresentInSearchShouldReturnFalseWhenOnlyItselfMatch()
	{
		when(cmsAdminItemService.findByTypeCodeAndName(any(), any(), any()))
				.thenReturn(new SearchResultImpl<>(asList(cmsItemModel), 1, 50, 0));
	...
	public void whenItemIsPresentInSearchShouldReturnFalseWhenOnlyOtherMatch()
	{
		...
		when(cmsAdminItemService.findByTypeCodeAndName(any(), any(), any()))
				.thenReturn(new SearchResultImpl<>(asList(otherCmsItemModel), 1, 50, 0));
	...
	public void whenItemIsPresentInSearchShouldReturnTrueWhenOtherItemMatch()
	{
		...
		otherCmsItemModel.setUid("other-uid");
		when(cmsAdminItemService.findByTypeCodeAndName(any(), any(), any()))
				.thenReturn(new SearchResultImpl<>(asList(cmsItemModel, otherCmsItemModel), 2, 50, 0));
	...
	{
		when(typeService.getComposedTypeForCode(ABTestCMSComponentContainerModel._TYPECODE)).thenReturn(composedTypeModel);
		...
		final String typeCode = predicate.findAbstractParentTypeCode(new ABTestCMSComponentContainerModel());
