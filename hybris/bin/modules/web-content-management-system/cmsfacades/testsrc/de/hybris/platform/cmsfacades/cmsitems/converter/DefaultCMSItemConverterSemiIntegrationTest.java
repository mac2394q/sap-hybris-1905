...
public class DefaultCMSItemConverterSemiIntegrationTest
{
	...
	public void shouldOnlyPopulateLanguagesWithReadPermission()
	{
		...
		when(cmsUserService.getReadableLanguagesForCurrentUser())
				.thenReturn(new HashSet<>(java.util.Collections.singletonList(FR)));
			...
			throws IOException
	{
		...
		when(cmsUserService.getWriteableLanguagesForCurrentUser())
				.thenReturn(new HashSet<>(java.util.Collections.singletonList(EN)));

		/*
		 * When a user doesn't have write permissions an all languages, the system only sets the values for which the user
		 * does have permissions. For the rest, the system uses the existing values. Since this test suite is used for
		 * both, model-to-data and data-to-model, it'd be easy to confuse the values that were actually updated with the
		 * original mock values. Thus, we set new values here to avoid any confusion.
		 */
		// --> Qualifier 2
