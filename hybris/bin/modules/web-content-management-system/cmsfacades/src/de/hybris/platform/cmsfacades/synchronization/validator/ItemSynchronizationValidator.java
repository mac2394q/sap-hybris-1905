...
import de.hybris.platform.cms2.common.predicates.ContentSlotPredicate;
...
public class ItemSynchronizationValidator implements Validator
{
    ...
    private ContentSlotPredicate cmsContentSlotPredicate;
    ...
    protected boolean isCmsItemApproved(final ItemModel itemModel)
    {
        ...
        List<CMSItemModel> relatedItems = getRelatedItemsService().getRelatedItems(cmsItem);
    ...
    {
        List<CMSItemModel> relatedPages = getRelatedItemsService().getRelatedItems(cmsItem).stream()
                .filter(this::isPage)
    ...
    {
        List<CMSItemModel> slots = items.stream().filter(this::isSlot).collect(Collectors.toList());
    ...
    @Required
    public void setCmsContentSlotPredicate(final ContentSlotPredicate cmsContentSlotPredicate)
    {
