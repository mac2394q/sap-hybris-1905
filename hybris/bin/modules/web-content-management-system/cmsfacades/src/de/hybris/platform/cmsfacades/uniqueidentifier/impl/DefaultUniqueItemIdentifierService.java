...
public class DefaultUniqueItemIdentifierService implements UniqueItemIdentifierService, InitializingBean
{
	...
	private final Map<String, UniqueIdentifierConverter> converterMap = new HashMap<>();
	...
	public Optional<ItemData> getItemData(final ItemModel itemModel)
	{
		...
		final Optional<ItemData> uniqueIdentifier = Optional.ofNullable(getConverterMap().get(itemType))
				.map(uniqueIdentifierConverter -> uniqueIdentifierConverter.convert(itemModel));
	...
	public Optional<ItemModel> getItemModel(final ItemData itemData)
	{
		...
		final Optional<ItemModel> uniqueItem = Optional.ofNullable(getConverterMap().get(itemType))
				.map(uniqueIdentifierConverter -> uniqueIdentifierConverter.convert(itemData));
	...
	{
		getUniqueIdentifierConverters().forEach(entry -> getConverterMap().put(entry.getItemType(), entry));
	}
	...
	public void setUniqueIdentifierConverters(final Set<UniqueIdentifierConverter> uniqueIdentifierConverters)
	{
	...
	@Required
	public void setCmsItemDataDataFactory(final ObjectFactory<ItemData> cmsItemDataDataFactory)
	{
