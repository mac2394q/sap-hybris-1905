...
public class DefaultComponentRenderingService implements ComponentRenderingService
{
	...
	@Override
	public SearchPageData<AbstractCMSComponentData> getAllComponents(final String categoryCode, final String productCode,
			final String catalogCode, final SearchPageData searchPageData)
	{
		validateParameters(categoryCode, productCode, catalogCode);
		final SearchPageData<AbstractCMSComponentModel> componentsSearchData = getCmsComponentService()
				.getAllAbstractCMSComponents(searchPageData);
		final RestrictionData restrictionData = getCmsDataFactory().createRestrictionData(categoryCode, productCode, catalogCode);
		return getComponentsData(componentsSearchData, restrictionData);
	}
	 ...
	 *           the {@link AbstractCMSComponentModel} object.
	 * @param restrictionData
	 *           the {@link RestrictionData} object determining whether the component is visible or not.
			...
			final RestrictionData restrictionData)
	{
		final Supplier<Optional<AbstractCMSComponentData>> convertComponentSupplier = () -> {
			if (getRenderingVisibilityService().isVisible(componentModel))
			{
				final AbstractCMSComponentData abstractCMSComponentData = getRenderingCacheService()
						.cacheOrElse(componentModel, () -> getComponentRenderingConverter().convert(componentModel));
				return Optional.of(abstractCMSComponentData);
			}
			else
			{
	...
	@Required
	public void setRenderingVisibilityService(final RenderingVisibilityService renderingVisibilityService)
	{
