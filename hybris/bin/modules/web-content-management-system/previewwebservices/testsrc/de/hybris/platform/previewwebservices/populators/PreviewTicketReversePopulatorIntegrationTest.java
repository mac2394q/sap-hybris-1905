...
public class PreviewTicketReversePopulatorIntegrationTest extends ServicelayerTransactionalTest
{
	...
	@Test
	public void shouldPopulateTargetWithSiteFromSiteIdProvidedFromSource()
	{
		final PreviewTicketWsDTO source = getPreviewTicketForLocalTestSiteWithSiteId();
		final PreviewDataModel target = new PreviewDataModel();
		previewTicketReversePopulator.populate(source, target);

		Assert.assertEquals(target.getActiveSite().getUid(), TEST_LOCAL_SITE);
	}
	...
	private PreviewTicketWsDTO getPreviewTicketForLocalTestSiteWithSiteId()
	{
		final PreviewTicketWsDTO previewTicketWsDTO = createPreviewTicketDTO(TEST_LOCAL_SITE, TEST_LOCAL_CATALOG,
				TEST_CATALOG_VERSION_STAGED, TEST_LOCAL_PAGE, null);
		previewTicketWsDTO.setSiteId(TEST_LOCAL_SITE);
		return previewTicketWsDTO;
	}
