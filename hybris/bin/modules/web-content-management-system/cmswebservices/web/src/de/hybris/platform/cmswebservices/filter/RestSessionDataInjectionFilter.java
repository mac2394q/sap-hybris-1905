 ...
 * <p>
 * This filter wraps the {@link ServletRequest} into a {@link MultiReadHttpServletRequest} such that the request body
 * can be read multiple times without closing the input stream. This is useful for
 * {@link de.hybris.platform.cmswebservices.interceptor.CatalogVersionPermissionInterceptor#preHandle()} when processing
 * POST requests.
 */
public class RestSessionDataInjectionFilter extends GenericFilterBean
{
	...
	{
		final HttpServletRequest httpRequest = new MultiReadHttpServletRequest((HttpServletRequest) request);
