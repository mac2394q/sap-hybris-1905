...
public class DefaultCMSNavigationServiceTest
{
	...
	public void setUp()
	{
		...
		rootNavigationModel.setUid(Cms2Constants.ROOT);
		...
		when(modelService.create(CMSNavigationEntryModel.class)).thenReturn(new CMSNavigationEntryModel(),
		...
		when(cmsNavigationDao.findRootNavigationNodes(Arrays.asList(catalogVersionModel)))
				.thenReturn(Arrays.asList(rootNavigationModel));
	}

	protected List<ItemModel> createContentPages()
	{
	...
	@Test
	public void shouldSetExistingSuperRootNodeOntoAOrphanNavigationNode()
	{
		firstNavigationModel.setParent(null);
		rootNavigationModel.setChildren(Arrays.asList(secondNavigationModel, thirdNavigationModel));
		CatalogVersionModel catalogVersionModel = firstNavigationModel.getCatalogVersion();

		when(cmsNavigationDao.findSuperRootNavigationNode(catalogVersionModel))
				.thenReturn(rootNavigationModel);

		cmsNavigationService.setSuperRootNodeOnNavigationNode(firstNavigationModel, catalogVersionModel);

		assertThat(rootNavigationModel.getChildren()).hasSize(3);
		assertThat(rootNavigationModel.getChildren().get(0)).isEqualTo(secondNavigationModel);
		assertThat(rootNavigationModel.getChildren().get(1)).isEqualTo(thirdNavigationModel);
		assertThat(rootNavigationModel.getChildren().get(2)).isEqualTo(firstNavigationModel);
		assertThat(firstNavigationModel.getParent()).isEqualTo(rootNavigationModel);
	}

	@Test
	public void shouldSetNonExitingSuperRootNodeOntoAOrphanNavigationNode()
	{
		firstNavigationModel.setParent(null);
		CatalogVersionModel catalogVersionModel = firstNavigationModel.getCatalogVersion();
		...
		when(cmsNavigationDao.findSuperRootNavigationNode(catalogVersionModel))
				.thenReturn(null);
		when(modelService.create(CMSNavigationNodeModel.class))
				.thenReturn(new CMSNavigationNodeModel());

		cmsNavigationService.setSuperRootNodeOnNavigationNode(firstNavigationModel, catalogVersionModel);

		CMSNavigationNodeModel rooNodeModel = firstNavigationModel.getParent();
		assertThat(rooNodeModel).isNotNull();
		assertThat(rooNodeModel.getUid()).isEqualTo(Cms2Constants.ROOT);
		assertThat(rooNodeModel.getChildren()).hasSize(1);
		assertThat(rooNodeModel.isVisible()).isFalse();
	}

	@Test
	public void shouldTestIFNodeIsASuperNavigationNode()
	{
		assertThat(cmsNavigationService.isSuperRootNavigationNode(rootNavigationModel)).isTrue();
		assertThat(cmsNavigationService.isSuperRootNavigationNode(firstNavigationModel)).isFalse();
	}
	...
	/**
	 * Test method for
	 * {@link DefaultCMSNavigationService#createCmsNavigationEntry(CMSNavigationNodeModel, String, ItemModel)}
	 */
	@Test
	public void shouldAddPage()
	{
		cmsNavigationService.createCmsNavigationEntry(thirdNavigationModel, "entry-page-name", firstContentPage);
	...
	/**
	 * Test method for {@link DefaultCMSNavigationService#removeNavigationEntryByUid(CMSNavigationNodeModel, String)}.
	 */
	@Test
	public void shouldRemoveEntryFromNavigationNode()
	{
		final String navigationEntryUid = secondNavigationModel.getEntries().stream().findFirst().get().getUid();
		cmsNavigationService.removeNavigationEntryByUid(secondNavigationModel, navigationEntryUid);
		assertThat(secondNavigationModel.getEntries()).isNotEmpty();
		assertThat(secondNavigationModel.getEntries()).hasSize(2);
		assertThat(extractCmsNavigationEntries(secondNavigationModel)).contains(secondContentPage, thirdContentPage);
	}
	...
	/**
	 * Test method for {@link DefaultCMSNavigationService#getRootNavigationNodes()}.
	 */
	@Test
	public void shouldGetAllRootNavigationNodes()
	{
		assertThat(cmsNavigationService.getRootNavigationNodes()).contains(rootNavigationModel);
	}
	 ...
	 * <p/>
	 */
	protected List<CMSNavigationEntryModel> wrapItemModel(final Collection<ItemModel> items,
			final CatalogVersionModel catalogVersion)
	{
		...
		for (final ItemModel item : items)
		{
			...
			cmsNavigationEntry.setUid(UUID.randomUUID().toString());
	 ...
	 */
	protected List<ItemModel> extractCmsNavigationEntries(final CMSNavigationNodeModel navigatioNode)
	{
