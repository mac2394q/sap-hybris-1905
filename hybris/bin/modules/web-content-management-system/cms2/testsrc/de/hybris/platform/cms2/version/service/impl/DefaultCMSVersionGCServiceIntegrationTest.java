...
public class DefaultCMSVersionGCServiceIntegrationTest extends ServicelayerTransactionalTest
{
	...
	@Test
	public void shouldReturnAllVersionsExcludedBy()
	{
		final PageableData pageableData = pageableDataFactory.getObject();
		pageableData.setCurrentPage(0);
		pageableData.setPageSize(5);

		final SearchResult<CMSVersionModel> searchResult = cmsVersionGCService.getVersionsExcludedBy(Arrays
				.asList(cmsVersionModel1, cmsVersionModel2), pageableData);
		...
		assertTrue(searchResult.getResult().stream()
				.noneMatch(result -> result.getUid().equals(VERSION_UID1)));
		...
		assertTrue(searchResult.getResult().stream()
				.anyMatch(result -> result.getUid().equals(VERSION_UID5)));
	}

	@Test
	public void shouldReturnAllVersionsWhenExcludedByIsEmpty()
	{
		final PageableData pageableData = pageableDataFactory.getObject();
		pageableData.setCurrentPage(0);
		pageableData.setPageSize(5);

		final SearchResult<CMSVersionModel> searchResult = cmsVersionGCService.getVersionsExcludedBy(Collections.emptyList(), pageableData);
