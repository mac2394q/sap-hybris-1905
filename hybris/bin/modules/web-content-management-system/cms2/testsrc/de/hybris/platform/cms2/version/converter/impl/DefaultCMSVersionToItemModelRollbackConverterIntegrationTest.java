...
@IntegrationTest
public class DefaultCMSVersionToItemModelRollbackConverterIntegrationTest extends AbstractCMSVersionIntegrationTest
{
	...
	public void testWillConvertCMSVersionModelToContentPageModelWithCustomAttributes()
	{
		...
		final ContentSlotData bodySlot = contentSlotsForPage.stream()
				.filter(relation -> BODY_SLOT.equals(relation.getContentSlot().getUid())).findFirst().orElse(null);

		// THEN
		Assert.assertThat(bodySlot.getUid(), is(BODY_SLOT));
		Assert.assertThat(bodySlot.getCMSComponents(), hasItem(allOf(hasProperty("uid", is(LINK_UUID)))));
	}
