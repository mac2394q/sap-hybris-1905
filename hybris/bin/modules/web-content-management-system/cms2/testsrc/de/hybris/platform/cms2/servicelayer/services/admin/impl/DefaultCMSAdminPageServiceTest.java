...
public class DefaultCMSAdminPageServiceTest
{
	...
	public void trashPageShouldFailWhenPageNotFound() throws CMSItemNotFoundException
	{
		// Given
		when(cmsPageDao.findPagesByIdAndPageStatuses(any(), any(), any())).thenReturn(Collections.emptyList());

		// When - Then
		pageService.trashPage(FAQ_PAGE_UID, globalCatalogVersionOnline);
	}

	@Test
	public void trashPageShouldSetPageStatusToDeletedAndSaveWhenPageFound() throws CMSItemNotFoundException
	{
		// Given
		when(cmsPageDao.findPagesByIdAndPageStatuses(any(), any(), any())).thenReturn(Arrays.asList(globalFaqPage));

		// When
		pageService.trashPage(FAQ_PAGE_UID, globalCatalogVersionOnline);

		// Then
		verify(globalFaqPage).setPageStatus(CmsPageStatus.DELETED);
		verify(modelService).save(globalFaqPage);
	}
