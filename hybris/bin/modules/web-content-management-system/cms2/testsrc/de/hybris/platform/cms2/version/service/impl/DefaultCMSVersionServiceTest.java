...
public class DefaultCMSVersionServiceTest
{
	...
	public void setup()
	{
		...
		when(pageTypePredicate.test(any())).thenReturn(false);
		when(contentPageTypePredicate.test(any())).thenReturn(false);
	...
	public void testRollbackVersionForAbstractPageModel()
	{
		...
		doReturn(Optional.of(cmsVersionModel)).when(versionService).getVersionByUid(VERSION_UID);
		doReturn(cmsVersionToModelRollbackConverter).when(versionService).getCmsVersionToModelRollbackConverter();
		doReturn(abstractPageModel).when(cmsVersionToModelRollbackConverter).convert(any());
		when(pageTypePredicate.test(abstractPageModel)).thenReturn(true);
		when(contentPageTypePredicate.test(abstractPageModel)).thenReturn(false);

		// WHEN
		final Optional<ItemModel> rollbackVersion = versionService.rollbackVersionForUid(VERSION_UID);

		// THEN
		assertThat(rollbackVersion.get(), equalTo(abstractPageModel));
		verify(cmsVersionToModelRollbackConverter).convert(any());
		verify(modelService).saveAll();
		verify(abstractPageModel).setApprovalStatus(CHECK);
	}
	...
	public void testRollbackedContentPageShouldBeAHomepageIfOriginalIsAHomepage()
	{
		...
		when(pageTypePredicate.test(contentPageModel)).thenReturn(true);
		when(contentPageTypePredicate.test(contentPageModel)).thenReturn(true);
		...
		verify(contentPageModel).setApprovalStatus(CHECK);
	}
	...
	public void givenPageVersionedIsNotCached_WhenFindPageVersionedByTransactionIdIsCalled_ThenThePageIsQueriedAndStoredInTheCache()
	{
		...
		// WHEN
		final Optional<AbstractPageModel> pageModel = versionService.findPageVersionedByTransactionId(TRANSACTION_ID);
