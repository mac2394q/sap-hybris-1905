...
public class DefaultSolrQueryConverterTest
{
	...
	public void setUp() throws Exception
	{
		...
		defaultSolrQueryConverter.setFacetSort(DefaultSolrQueryConverter.FacetSort.COUNT);
		defaultSolrQueryConverter.setFacetSearchContextFactory(facetSearchContextFactory);
		defaultSolrQueryConverter.setLegacyFacetSearchQueryConverter(legacyFacetSearchQueryConverter);
		...
		given(fieldNameTranslator.translate(searchQuery, TEST_FACET_CATEGORY_NAME, FieldNameProvider.FieldType.INDEX))
				.willReturn(TEST_FACET_CATEGORY_TRANSLATED_NAME);
		given(legacyFacetSearchQueryConverter.convert(Mockito.any(SearchQueryConverterData.class))).willReturn(new SolrQuery());
	}
	...
	public void testConvertQueryWithSimpleCoupledFields() throws Exception
	{
		...
		assertTrue("fq not as expected", result
				.contains("q=(((catalogId:TestCatalog1) AND (version:Online)) OR ((catalogId:TestCatalog2) AND (version:Staged)))"));
	}
	...
	public void testConvertQueryWithComplicatedCoupledFields() throws Exception
	{
		...
		assertTrue("q not as expected", result.contains(
				"q=(((catalogId:(TestCatalog1A AND TestCatalog1B AND TestCatalog1C)) AND (version:(version1 OR version2 OR version3))) AND ((catalogId:(TestCatalog2A OR TestCatalog2B OR TestCatalog2C)) OR (version:(versionA AND versionB AND versionC))))"));
	}
}
