...
class ProductTests extends BaseWSTest {
	...
	static final NUMBER_OF_ALL_INDEXED_PRODUCTS = 44
	static final NUMBER_OF_ALL_PRODUCTS = 897
	static final STORE_NAME = "WS-Shinbashi"
	static final SUGGESTION_RESULTS = ['targus','tapes']
	...
	void testSearchProductsAutoSuggestXML() {
		...
		assert SUGGESTION_RESULTS.contains(response.suggestion[0])
		assert SUGGESTION_RESULTS.contains(response.suggestion[1])
	}
	...
	void testSearchProductsAutoSuggestJSON() {
		...
		assert SUGGESTION_RESULTS.contains(response.suggestions[0].value)
		assert SUGGESTION_RESULTS.contains(response.suggestions[1].value)
	}
	...
	void testProductExtendedDetailsSpecificSJON() {
		...
		assert response.purchasable
		assert response.stock.stockLevelStatus.value == 'inStock'
	...
	void testGetStockLevelForStore() {
		...
		assert response.stockLevelStatus.value == "inStock"
