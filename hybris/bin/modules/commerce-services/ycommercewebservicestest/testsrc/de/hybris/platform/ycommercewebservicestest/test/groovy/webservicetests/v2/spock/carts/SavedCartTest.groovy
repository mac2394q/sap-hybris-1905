...
class SavedCartTest extends AbstractCartTest {
	...
	def "Registered and authorized customer saves a cart and then restores it : #format"(){
		...
		sleep(1000) // related to problem RAY-192 with MySQL database

		when: "authorized customer wants to restore the saved cart"
		def response	= getResponseFromRestoringSavedCart(restClient, customer, cart, format)
		...
		with(response){
			...
			isEmpty(response.data.savedCartData.saveTime)
	...
	def "Customer flags another customer's cart for deletion: #format"(){
		...
		HttpResponseDecorator response = restClient.post(
				path : getBasePathWithSite() + '/users/'+ customer2.id +'/carts/' + savedCart.savedCartData.code +
                        '/flagForDeletion',
				contentType: format,
				query : ['fields' : FIELD_SET_LEVEL_FULL],
				requestContentType: URLENC
	...
	def "Registered and authorized customer gets a saved cart : #format"(){
		...
		def response = restClient.get(
				path : getBasePathWithSite() + '/users/'+ customer.id +'/carts/'+ savedCart.savedCartData.code + '/savedcart',
				query : ['fields' : FIELD_SET_LEVEL_FULL],
				contentType: format,
				requestContentType: URLENC
				)
	...
	def "Registered and authorized customer trying to get a saved cart providing invalid user id: #format"(){
		...
		def response = restClient.get(
				path : getBasePathWithSite() + '/users/'+ invalidUserId +'/carts/' + savedCart.savedCartData.code + '/savedcart',
				query : ['fields' : FIELD_SET_LEVEL_FULL],
				contentType: format,
				requestContentType: URLENC
				)
