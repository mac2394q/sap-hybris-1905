...
class CartResourceTest extends AbstractCartTest {
	...
	def "Registered and authorized customer creates cart, add one product and checks for tax details on cart: #format"(){
		given: "a registered customer adds product into cart"
		def val = createAndAuthorizeCustomerWithCart(restClient, JSON)
		def customer = val[0]
		def cart = val[1]
		addProductToCartOnline(restClient, customer, cart.code, PRODUCT_POWER_SHOT_A480, 1, format)

		when: "authorized customer wants to retrieve carts"
		HttpResponseDecorator response = restClient.get(
				path: getBasePathWithSite() + '/users/' + customer.id +'/carts/'+cart.code,
		...
		with(response){
			...
			data.totalPrice.value.toDouble() > 0.0
			if(!data.net.toBoolean()) assert data.totalTax.value.toDouble() > 0.0 // when net is false, tax should be > 0.0
			if(data.net.toBoolean()) assert data.totalTax.value.toDouble() == 0.0 // when net is true, tax should be 0.0
		}
