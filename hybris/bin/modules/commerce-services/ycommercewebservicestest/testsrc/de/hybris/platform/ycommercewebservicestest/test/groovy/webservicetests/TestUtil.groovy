...
class TestUtil {
	...
	public getSecureConnection(path, method = 'GET', accept = 'XML', code = HttpURLConnection.HTTP_OK, body = null, cookie = null, auth = null, contentType = "application/x-www-form-urlencoded", consents = null) {
		fakeSecurity()
		...
		if (consents) {
			con.setRequestProperty('X-Anonymous-Consents', consents)
		}
	...
	public verifyJSON(jsonString, fail = config.FAIL_ON_NAMING_CONVENTION_ERROR) {
		assert (jsonString.contains('HybrisDynamicEnumValueSerializedForm') || !jsonString.contains('de.hybris')): "JSON Serialization issue - Check XStream Config! ${jsonString}"
	...
	public verifyXML(xmlString, fail = config.FAIL_ON_NAMING_CONVENTION_ERROR) {
		...
		assert (xmlString.contains('HybrisDynamicEnumValueSerializedForm') || !xmlString.contains('de.hybris')): "XML Serialization issue - Check XStream Config! ${xmlString}"
