...
public class CheckoutConfigurationBackendTools
{
	...
	protected JCoParameterList executeBackendAccess(final JCoConnection jConn, final String searchhelp) throws BackendException
	{
		...
		if ("S&013".equals(returnStructure.getString("TYPE")))
		{
			...
			sapLogger.log(Severity.WARNING, LogCategories.APPS_COMMON_CONFIGURATION, "Searchhelp doesn't exist", new Object[]
			{ logMessage });
		}
			...
			final String descriptionField, final boolean withFallbackDesc, final Map<String, String> searchResult)
	{
		...
		for (int i = 0; i < jFieldList.getNumRows(); i++)
		{
			...
			{
				sapLogger.log(Severity.ERROR, LogCategories.APPS_BUSINESS_LOGIC, "Exception during access to Customizing",
						new Object[]
						{ ex.toString() });
				sapLogger.traceThrowable(Severity.DEBUG, "Exception during access to Customizing", ex);
				sapLogger.exiting();
		...
		if (intValueFieldEnd == 0)
		{
			...
			sapLogger.log(Severity.ERROR, LogCategories.APPS_BUSINESS_LOGIC, "Exception during access to Customizing", new Object[]
			{ ex.toString() });
			sapLogger.traceThrowable(Severity.DEBUG, "Exception during access to Customizing", ex);
			sapLogger.exiting();
