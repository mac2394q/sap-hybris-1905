...
class ConsumedDestinationBuilder {
    ...
    private static ConsumedDestinationModel consumedDestination(String id, String url, EndpointModel endpoint, AbstractCredentialModel credential, DestinationTargetModel target, Map params) {
        ...
        importImpEx(
                'INSERT_UPDATE ConsumedDestination; id[unique = true]; url              ; endpoint                      ; credential                        ; destinationTarget         ; additionalProperties(key, value)[map-delimiter = |]',
                "                                 ; $idVal           ; ${deriveUrl(url)}; ${deriveEndpoint(endpoint).pk}; ${deriveCredential(credential).pk}; ${deriveTarget(target).pk}; ${serializeParameters(params)}")
